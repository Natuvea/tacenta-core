...
fn replacement_keys_make_room_at_the_exact_store_edge() {
    ...
    for n in 5_000..6_997 {
        b.skipped.push(Skipped { epoch: 0, n, key: [0x32; 32] });
    }
...
#[test]
fn a_held_key_at_the_chain_number_is_not_replaced() {
    // The re-derived range is (ch.n, upto]. A persisted key at the lower
    // endpoint is outside that range and must survive byte-for-byte.
    let mut b = State::init_bob(&sk());
    let held = [0xC7; 32];
    b.skipped.push(Skipped { epoch: 0, n: 0, key: held });
    b.skip_message_keys(0, 2).unwrap();
    let lower = b
        .skipped
        .iter()
        .find(|s| s.epoch == 0 && s.n == 0)
        .expect("lower-endpoint key survives");
    assert_eq!(lower.key, held);
}
