...
| P6 | Session/prekey operation model and L2 evidence | CLOSED at the recorded bounded L2 target; reopen only on its decision triggers. |
...
| MU-04 | BLOCKING | Evidence/CI implementer | CI emits an honest, versioned inventory of the selected candidate's required checks and evidence. | Each CI job writes an unsigned, candidate-produced checkpoint after its checks. Its literal `status: pass` means only that execution reached that step; it is not an authenticated GitHub job conclusion, and the later digest/archive cannot make it one. A separate `workflow_run` workflow executes default-branch code, queries GitHub's API for the completed CI run and requires the exact candidate, event, ten job names and conclusions. Its local controls cover missing jobs, failure, wrong head and push/PR sign-off handling. Neither the fixed push aggregation path nor the trusted hosted-conclusion workflow has yet produced a retained successful `main` artifact. | After merge, retain and verify the first default-branch-derived conclusion artifact for a clean `main` candidate, then bind that artifact into the final evidence pack. |
...
| `tooling/check-workflows.sh` | Workflow and composite-action files parse and obey repository security rules. | 65 case files under `tooling/tests/check-workflows-cases`, including composite-action tag-pin and download-to-shell refusals. GitHub's repository-level SHA-pinning requirement is also enabled. | Retain the current cases and the repository setting. |
