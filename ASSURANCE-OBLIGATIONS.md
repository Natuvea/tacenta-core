...
| MU-04 | BLOCKING | Evidence/CI implementer | CI emits an honest, versioned inventory of the selected candidate's required checks and evidence. | Each CI job writes an unsigned, candidate-produced checkpoint after its checks. Its literal `status: pass` means only that execution reached that step; it is not an authenticated GitHub job conclusion, and the later digest/archive cannot make it one. The aggregation job downloads checkpoints only after its declared dependencies succeed, applies push/PR conditional sign-off rules, then builds and validates the inventory. Local refusal controls cover missing, skipped, falsely inapplicable, foreign-candidate, mismatched-event, duplicate and missing conditional records. The push dependency fix has not yet produced a retained successful `main` artifact. | Derive the final candidate's conclusions from the GitHub Actions API in trusted/default-branch code or add verifiable artifact provenance; retain the hosted run and artifact, and record hosted-only controls. |
