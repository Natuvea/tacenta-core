...
| Persisted formats: session | Component table still below target because stored-format differential checks remain absent; P4 fixtures now pin tag boundary branches, failed-Braid exemption, Braid-half role agreement, ratchet private/public relation, unanswered-role exclusion and reachable inner invariant refusals. | BLOCKING | P5 | Stored-format differential checks where in shared model domain, or explicit scoped exclusions for fields outside the model. |
...
| Practice 9: differential testing | The prekey store's shared structural stored-format domain is now driven through both sides; session stored formats remain modelled but not driven, and the Braid state machine remains partly outside differential vectors. | BLOCKING | P5/P7 | Required differential run reaches intended versions/branches/refusals, or explicit scoped exclusions. |
| Gate 1 | Every component must meet target or have a recorded target decision. Several rows above do not. | BLOCKING | P1/P7/P6/P5 | Component obligation matrix closed or amended by recorded decisions. |
| Gate 2 | Gap register must have no open BLOCKING rows and no unresolved AMBIGUOUS rows. | BLOCKING | P1/P3/P5/P6/P7 | `GAP-REGISTER.md` rows classified and closed/decided as required. |
