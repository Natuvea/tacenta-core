...
| --- | --- | --- | --- | --- |
| Sparse post-quantum ratchet | `P7-SPARSE-BRAID-INVENTORY.md` maps the scoped operations/invariants to model, generated differential, persisted-vector and clean-room reader evidence. The differential harness now proves the previously unexercised observable effect of retirement: after epoch 0 retires, an operation naming it refuses as `NoChain` and leaves canonical state unchanged. The vector target remains partial because the generated run does not separately classify every refusal combination as a fixed vector. | BLOCKING | P7 | Decide which remaining generated refusal combinations need fixed vectors, then add them or record the target decision. |
| ML-KEM Braid | `P7-SPARSE-BRAID-INVENTORY.md` separates stored-state coverage for tags 0 and 5--11 and `Ct2Sampled` boundaries from the delegated valid key-pair layout for tags 1--4. The differential harness drives an empty `NoHeaderReceived` state through transition (6) with a persisted-key header MAC and a one-byte mutation, reaching `HeaderReceived` and `Failed`. A separate real-ML-KEM runner control drives transition (5): a complete `ct2 || MacCt` stream emits the epoch key, while a one-byte mutation in its final MAC-only systematic codeword emits none and reaches `Failed`. Other state-machine branches remain unexercised. | BLOCKING | P7 | Decide whether the remaining KEM-backed branches need committed operation vectors or record a narrow target decision; retain the delegated-layout exception only for its exact key-pair fields. |
| Erasure code | Component table says L3 while target is L4; full decoder refinement is deferred but no decision lowers the target. `ERASURE-DECODER-REFINEMENT.md` records that the current `DecoderRefines` relation is an assumed opaque-boundary agreement. Model duplicate/completed effects are proved, and `Tacenta.ErasureT3.decoder_add_chunk_rejects_complete` proves the completed effect for the separately translated leaf API. | BLOCKING | P7 | Link the leaf result to Braid's opaque erasure dependency; bridge duplicate and completed observations through the real decoder's translated API; prove the concrete `DecoderRefines` preservation statement; then either complete L4 evidence or record a target decision. |
...
| Practice 9: differential testing | The prekey store's and session's shared structural stored-format domains are now driven through both sides; Braid covers stored tags, `Ct2Sampled` boundaries, and the authenticated-header transition, but its state machine remains partial. | BLOCKING | P7 | Required differential run reaches intended versions/branches/refusals, or explicit scoped exclusions. |
