...
| ML-KEM Braid | `P7-SPARSE-BRAID-INVENTORY.md` separates stored-state coverage for tags 0 and 5--11 and `Ct2Sampled` boundary transitions from the delegated valid key-pair layout for tags 1--4. MAC-valid/invalid traffic and the other state-machine branches remain unexercised. | BLOCKING | P7 | Specify MAC vector provenance; add accepted-neighbour/failure-state MAC evidence and one feasible additional transition; retain the delegated-layout exception only for its exact key-pair fields. |
