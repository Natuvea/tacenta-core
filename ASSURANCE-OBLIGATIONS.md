...
| --- | --- | --- | --- | --- |
| Sparse post-quantum ratchet | `P7-SPARSE-BRAID-INVENTORY.md` records generated differential coverage of initialization, epoch advancement, send/receive, skipped-key recovery, retained-epoch retirement and ceilings. The vector target remains partial because there is no itemized mapping from every scoped operation/invariant to its evidence. | BLOCKING | P7 | Publish the scoped operation/invariant map, identify a real missing item, then add its vector/verifier or record a target decision. |
