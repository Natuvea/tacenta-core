...
| ML-KEM Braid | `P7-SPARSE-BRAID-INVENTORY.md` separates stored-state coverage for tags 0 and 5--11 and `Ct2Sampled` boundaries from the delegated valid key-pair layout for tags 1--4. The differential harness now drives an empty `NoHeaderReceived` state through transition (6) with a persisted-key header MAC and a one-byte MAC mutation, reaching `HeaderReceived` and `Failed`. Ciphertext-MAC traffic and other state-machine branches remain unexercised. | BLOCKING | P7 | Specify ciphertext-MAC provenance; add accepted-neighbour/failure-state ciphertext evidence; identify another no-KEM transition or record a narrow fixture/scope decision; retain the delegated-layout exception only for its exact key-pair fields. |
