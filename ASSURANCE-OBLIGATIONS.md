...
| Persisted formats: prekey store | Component table still below target because stored-format differential checks remain absent; P4 fixtures now pin older versions, `previous_kem` and stored-signature refusal. | BLOCKING | P5 | Stored-format differential checks where in shared model domain, or explicit scoped exclusions for fields outside the model. |
