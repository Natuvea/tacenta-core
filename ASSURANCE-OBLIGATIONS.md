...
| Session orchestration and prekey store | Component table says L1 while target is L2 by recorded decision. Stored-format modelling alone does not model operations. | BLOCKING | P6 | `tacenta-model/SESSION-OPERATION-MODEL.md` records the initial operation inventory, abstraction contract and operation-to-requirement matrix; closure still requires model-generated scenarios, concrete Rust execution and reader coverage for the agreed L2 surface. |
