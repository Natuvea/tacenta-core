...
| Practice 8: evidence traceability | Structural ID spine is checked; proof/vector/test evidence links are checked for the migrated authentication and confidentiality entries, but FS/PCS requirements are still outside the index. | BLOCKING | P2 | Complete the machine-readable requirement evidence index, require entries for every requirement, and retain reference-check negative controls. |
