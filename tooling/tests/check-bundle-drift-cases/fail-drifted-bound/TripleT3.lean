-- expect: BOUND DRIFT
-- The classical receive clause keeps `events < U32.max` where the leaf
-- theorem asks for `events + 1 < U32.max`. This is the shape of both
-- historical drifts, and the direction that matters: the bundle would
-- be asserting the refinement under a hypothesis nothing proved.
