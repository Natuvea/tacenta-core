-- expect: no clause mirrors `Tacenta.T3.init_receiver_refines`
-- The `init_receiver` clause is gone. Comparing what is left would pass,
-- so the bundle's declared list of leaf theorems is what catches it.
