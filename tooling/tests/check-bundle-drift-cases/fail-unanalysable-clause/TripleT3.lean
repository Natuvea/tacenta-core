-- expect: state field with no transport entry: pn
-- The classical receive clause states its clock bound over a field the
-- substitution table has no real-side path for. Normalisation cannot
-- decide whether the two sides speak of the same thing, so this is a
-- failure rather than a comparison of the clauses around it.
