...
-- Translation/SpqrT3.lean. Do not edit: `port-unit-proofs.sh --check`
...
theorem skipped_clone_spec (s : Skipped) :
  ...
  Tacenta.UnitSpqrT1.skipped_clone_spec s
...
theorem send_refines (hkr : SpqrHkdfAgrees)
    ...
    (hz : Tacenta.UnitSpqrT1.ZeroizeTotal) (hopt : Tacenta.UnitSpqrT1.OptionCloneTotal)
    {s : State} {m : Model.SparseRatchet.State} (hrel : StateRefines s m)
  ...
  · -- The model's `maybe_advance` succeeds with `m'`: proceed to look up the chain.
    ...
    · -- Chains for this epoch exist: clone them and check the sending side.
      ...
      have hfindeq : Model.SparseRatchet.findChains m' sending_epoch.val = some (chainsOf cs) := by
        ...
        simpa using h2.symm
      step with Tacenta.UnitSpqrT1.chains_clone_spec hopt cs
      rename_i cs1_post
      ...
      · -- Ready to send: derive the next chain key and message key.
        ...
        step with hopt Chain.Insts.CoreCloneClone cs1.receive
          (fun x _ => Tacenta.UnitSpqrT1.chain_clone_spec x)
...
theorem VecAppendAgrees.total (h : VecAppendAgrees) : Tacenta.UnitSpqrT1.VecAppendTotal := by
  intro T A v w hlen
...
theorem skip_message_keys_refines (hkr : SpqrHkdfAgrees) (hz64 : ZeroizingRoundTrips64)
    (hret : VecRetainAgrees)
    (happ : VecAppendAgrees) (hz : Tacenta.UnitSpqrT1.ZeroizeTotal)
  ...
  · rw [ho] at o_post
    ...
    · simp only [chainsOf, hcsr, Option.map_some]
      ...
      · -- Room for the walk: derive the forward keys, retain everything else,
        ...
        obtain ⟨v1, w⟩ := r
        dsimp only at hrveq ⊢
        step with hopt Chain.Insts.CoreCloneClone cs.send (fun x _ => Tacenta.UnitSpqrT1.chain_clone_spec x)
...
theorem receive_refines_continuation (hkr : SpqrHkdfAgrees) (hz64 : ZeroizingRoundTrips64)
    (hret : VecRetainAgrees)
    (happ : VecAppendAgrees) (hrm : RemoveSkippedAtAgrees) (hz : Tacenta.UnitSpqrT1.ZeroizeTotal)
    (hopt : Tacenta.UnitSpqrT1.OptionCloneTotal)
