...
namespace Tacenta.SessionUnitRatchetImportInv
...
theorem array_eq_eq {N : Std.Usize} (a b : Array Std.U8 N) :
  ...
  obtain ⟨v, hv, hiff⟩ := Std.WP.spec_imp_exists (Tacenta.SessionUnitT3.array_eq_val a b)
  rw [hv]
  cases v <;> simp_all
...
loops by `is_canonical_x25519`, whose value `Tacenta.SessionUnitT1.is_canonical_x25519_spec`
proves. -/
namespace Ratchet
...
theorem canonical_eq (k : Array Std.U8 32#usize) :
    is_canonical_x25519 k = ok (Tacenta.SessionUnitT1.canonicalX25519 k) := by
  obtain ⟨r, hr, hre⟩ := Std.WP.spec_imp_exists (Tacenta.SessionUnitT1.is_canonical_x25519_spec k)
  rw [hr, hre]
...
def storeOk : List SkippedKey → Std.U32 → Bool
  | [], _ => true
  | a :: rest, ev =>
      decide (a.stored_at.val ≤ ev.val) && Tacenta.SessionUnitT1.canonicalX25519 a.dh
...
def keysOk (s : State) : Bool :=
  Tacenta.SessionUnitT1.canonicalX25519 s.dhs_pub &&
    match s.dhr_pub with
    | none => true
    | some k => Tacenta.SessionUnitT1.canonicalX25519 k
...
structure Inv (s : State) : Prop where
  ...
  dhs_canonical : Tacenta.SessionUnitT1.canonicalX25519 s.dhs_pub = true
  dhr_canonical : ∀ k, s.dhr_pub = some k → Tacenta.SessionUnitT1.canonicalX25519 k = true
  store_canonical : ∀ e ∈ s.skipped.val, Tacenta.SessionUnitT1.canonicalX25519 e.dh = true
...
theorem storeOk_iff (L : List SkippedKey) (ev : Std.U32) :
    storeOk L ev = true ↔
      ((∀ e ∈ L, e.stored_at.val ≤ ev.val) ∧
        (∀ e ∈ L, Tacenta.SessionUnitT1.canonicalX25519 e.dh = true) ∧
...
theorem canonical_zeros : Tacenta.SessionUnitT1.canonicalX25519 (Std.Array.repeat 32#usize 0#u8) = true := by
  decide
...
theorem inv_gives_store_is_map (s : State) (m : Model.State.State)
    (hR : Tacenta.SessionUnitT3.StateR s m) (hi : Inv s) (mh : Model.State.Header) :
    (m.skipped.filter (Tacenta.SessionUnitT3.matchesHeader mh)).length ≤ 1 := by
  have hpm : m.skipped.Pairwise (fun a b =>
    ...
    intro a b hab
    simp only [Tacenta.SessionUnitT3.skippedOf, ne_eq, Prod.mk.injEq, not_and]
    intro h1 h2
    exact hab ⟨Tacenta.SessionUnitT3.keyOf_inj h1, by scalar_tac⟩
  refine length_filter_le_one_of_pairwise (k := (mh.dh, mh.n)) hpm ?_
  rintro ⟨dh, n, rest⟩ _ hpe
  simp only [Tacenta.SessionUnitT3.matchesHeader, Bool.and_eq_true, beq_iff_eq] at hpe
  simp [hpe.1, hpe.2]
...
theorem decoded_receive_no_panic (h : Tacenta.SessionUnitT1.HmacTotal) (hk : Tacenta.SessionUnitT1.HkdfTotal)
    (hz : Tacenta.SessionUnitT1.ZeroizingTotal) (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    ...
    Tacenta.SessionUnitT1.NoPanic (receive s header dh_out_recv dh_out_send new_dhs_pub) :=
  Tacenta.SessionUnitT1.receive_no_panic h hk hz hrm s header dh_out_recv dh_out_send new_dhs_pub
...
means. -/
theorem decoded_receive_refines (h : Tacenta.SessionUnitT3.HmacAgrees) (hk : Tacenta.SessionUnitT3.HkdfAgrees)
    (hz : Tacenta.SessionUnitT3.ZeroizingRoundTrips) (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    [Tacenta.SessionUnitT1.DerivedKeysModel]
    (bytes : Slice Std.U8) (s : State)
    (hdec : State.from_bytes bytes = ok (core.result.Result.Ok s))
    (hclock_unparked : s.events.val + 1 < Std.U32.max)
    (m : Model.State.State) (hR : Tacenta.SessionUnitT3.StateR s m)
    (hdr : Header) (mh : Model.State.Header) (hH : Tacenta.SessionUnitT3.HeaderR hdr mh)
    ...
    receive s hdr dh_out_recv dh_out_send new_dhs_pub ⦃ fun r =>
      ∀ mk, r.1 = core.result.Result.Ok mk →
        ∃ m', Model.Ratchet.receive m mh (Tacenta.SessionUnitT3.keyOf dh_out_recv)
              (Tacenta.SessionUnitT3.keyOf dh_out_send) (Tacenta.SessionUnitT3.keyOf new_dhs_pub)
                = some (m', Tacenta.SessionUnitT3.keyOf mk)
          ∧ Tacenta.SessionUnitT3.StateR r.2 m' ⦄ :=
  have hinv := from_bytes_establishes_inv bytes s hdec
  Tacenta.SessionUnitT3.receive_refines h hk hz hrm s m hR hdr mh hH
...
theorem inv_gives_store_is_map (s : State) (m : Model.SparseRatchet.State)
    (hR : Tacenta.SessionUnitSpqrT3.StateRefines s m) (hi : Inv s) (e n : Std.U64) :
  ...
  have hpm : m.skipped.Pairwise (fun a b =>
    ...
    intro a b hab
    simp only [Tacenta.SessionUnitSpqrT3.skippedOf, ne_eq, Prod.mk.injEq, not_and]
    intro h1 h2
...
theorem decoded_receive_no_panic (hret : Tacenta.SessionUnitSpqrT1.VecRetainTotal)
    (hrk : Tacenta.SessionUnitSpqrT1.KdfRkTotal) (hz : Tacenta.SessionUnitSpqrT1.ZeroizeTotal)
    (hkdf : Tacenta.SessionUnitSpqrT1.KdfCkTotal) (hopt : Tacenta.SessionUnitSpqrT1.OptionCloneTotal)
    (hrm : Tacenta.SessionUnitSpqrT1.RemoveSkippedAtTotal) (happ : Tacenta.SessionUnitSpqrT1.VecAppendTotal)
    (bytes : Slice Std.U8) (s : State)
    (hdec : State.from_bytes bytes = ok (core.result.Result.Ok s))
    (receiving_epoch n : Std.U64) (out : Option Output) :
    State.receive s receiving_epoch out n ⦃ fun _ => True ⦄ :=
  have hinv := from_bytes_establishes_inv bytes s hdec
  Tacenta.SessionUnitSpqrT1.receive_no_panic hret hrk hz hkdf hopt hrm happ s receiving_epoch n out
