...
-- Translation/UnitTripleT3.lean. Do not edit: `port-session-unit-proofs.sh --check`
...
eight-leaf Session translation unit (`Translation/TacentaTripleUnit.lean`), where the
...
`Tacenta.SessionUnitT3` and `Tacenta.SessionUnitSpqrT3`, the inner refinements restated about
...
* the namespace, `Tacenta.SessionUnitTripleT3`, and the `open`, which names the unit's
...
namespace Tacenta.SessionUnitTripleT3
...
theorem TripleHkdfAgrees.total (h : TripleHkdfAgrees) : Tacenta.SessionUnitT1.HkdfTotal :=
...
theorem ZeroizingRoundTrips.total (h : ZeroizingRoundTrips) :
    Tacenta.SessionUnitT1.ZeroizingTotal := by
  intro inst
...
attribute [-step] Tacenta.SessionUnitTripleT1.combine_step Tacenta.SessionUnitTripleT1.zeroize_step
...
def RatchetAgreesFor (α : tacenta_ratchet.State → Model.State.State) : Prop :=
  ...
  -- mirrors: Tacenta.SessionUnitT3.init_sender_refines in UnitT3.lean
  ...
  -- mirrors: Tacenta.SessionUnitT3.init_receiver_refines in UnitT3.lean
  ...
  -- mirrors: Tacenta.SessionUnitT3.send_refines in UnitT3.lean
...
def SpqrAgreesFor (β : tacenta_spqr.State → Model.SparseRatchet.State) : Prop :=
  ...
  -- mirrors: Tacenta.SessionUnitSpqrT3.send_refines in UnitSpqrT3.lean
  ...
  -- mirrors: Tacenta.SessionUnitSpqrT3.receive_refines in UnitSpqrT3.lean
...
def ratchetAbs (s : tacenta_ratchet.State) : Model.State.State where
  dhsPub  := Tacenta.SessionUnitT3.keyOf s.dhs_pub
  dhrPub  := s.dhr_pub.map Tacenta.SessionUnitT3.keyOf
  rk      := Tacenta.SessionUnitT3.keyOf s.rk
  cks     := s.cks.map Tacenta.SessionUnitT3.keyOf
  ckr     := s.ckr.map Tacenta.SessionUnitT3.keyOf
  ns      := s.ns.val
  nr      := s.nr.val
  pn      := s.pn.val
  skipped := s.skipped.val.map Tacenta.SessionUnitT3.skippedOf
  events  := s.events.val
  labels  := Tacenta.SessionUnitT3.labelsOf s.labels

theorem ratchetAbs_stateR (s : tacenta_ratchet.State) :
    Tacenta.SessionUnitT3.StateR s (ratchetAbs s) :=
...
theorem ratchetAbs_eq {s : tacenta_ratchet.State} {m : Model.State.State}
    (h : Tacenta.SessionUnitT3.StateR s m) : ratchetAbs s = m := by
...
def spqrAbs (s : tacenta_spqr.State) : Model.SparseRatchet.State where
  rk        := Tacenta.SessionUnitSpqrT3.keyOf s.rk
  epoch     := s.epoch.val
  chains    := s.chains.val.map Tacenta.SessionUnitSpqrT3.chainsEntryOf
  skipped   := s.skipped.val.map Tacenta.SessionUnitSpqrT3.skippedOf
  direction := Tacenta.SessionUnitSpqrT3.directionOf s.direction

theorem spqrAbs_refines (s : tacenta_spqr.State) :
    Tacenta.SessionUnitSpqrT3.StateRefines s (spqrAbs s) :=
  ⟨rfl, rfl, rfl, rfl, rfl⟩

theorem spqrAbs_eq {s : tacenta_spqr.State} {m : Model.SparseRatchet.State}
    (h : Tacenta.SessionUnitSpqrT3.StateRefines s m) : spqrAbs s = m := by
...
theorem spqr_init_refines (h : Tacenta.SessionUnitSpqrT3.SpqrHkdfAgrees)
    (hz : Tacenta.SessionUnitSpqrT3.ZeroizingRoundTrips96) (sk : Slice Std.U8)
    (d : tacenta_spqr.Direction) :
    ∃ r, tacenta_spqr.State.init sk d = ok r ∧
      spqrAbs r = Model.SparseRatchet.init (Tacenta.SessionUnitSpqrT3.sliceOf sk)
        (Tacenta.SessionUnitSpqrT3.directionOf d) := by
  obtain ⟨⟨rk, k1, k2⟩, hr, heq⟩ :=
    Std.WP.spec_imp_exists (Tacenta.SessionUnitSpqrT3.kdf_init_refines h hz sk)
  unfold tacenta_spqr.State.init
  cases d <;> simp [hr, lift, alloc.slice.Slice.into_vec, Array.to_slice, Array.make,
    spqrAbs, Model.SparseRatchet.init, ← heq, Tacenta.SessionUnitSpqrT3.chainsEntryOf,
...
theorem ratchet_agrees_for (hopt : Tacenta.SessionUnitSpqrT1.OptionCloneTotal)
    (hmac : Tacenta.SessionUnitT3.HmacAgrees) (hkdf : Tacenta.SessionUnitT3.HkdfAgrees)
    (hzr : Tacenta.SessionUnitT3.ZeroizingRoundTrips) (hvr : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    [Tacenta.SessionUnitT1.DerivedKeysModel] :
    RatchetAgreesFor ratchetAbs := by
  unfold RatchetAgreesFor
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro s
    exact ⟨s, Tacenta.SessionUnitTripleT1.ratchet_state_clone_id hopt s, rfl⟩
  · intro s
  ...
  · intro ec our_pub peer_pub dh_out labels
    obtain ⟨r, hr, hR⟩ := Std.WP.spec_imp_exists
      (Tacenta.SessionUnitT3.init_sender_refines hkdf hzr ec our_pub peer_pub dh_out labels)
    refine ⟨r, hr, ?_⟩
    rw [ratchetAbs_eq hR]
    cases labels; rfl
  · intro ec our_pub labels
    obtain ⟨r, hr, hR⟩ := Std.WP.spec_imp_exists
      (Tacenta.SessionUnitT3.init_receiver_refines ec our_pub labels)
    refine ⟨r, hr, ?_⟩
    rw [ratchetAbs_eq hR]
    cases labels; rfl
  · intro s
    obtain ⟨r, hr, hok, herr⟩ := Std.WP.spec_imp_exists
      (Tacenta.SessionUnitT3.send_refines hmac s (ratchetAbs s) (ratchetAbs_stateR s))
  ...
  · intro s hdr mh hH dh_out_recv dh_out_send new_dhs_pub hone hs hroom
    obtain ⟨r, hr, hpost⟩ := Std.WP.spec_imp_exists
      (Tacenta.SessionUnitT3.receive_refines hmac hkdf hzr hvr s (ratchetAbs s) (ratchetAbs_stateR s)
        hdr mh ⟨hH.dh, hH.pn, hH.n⟩ dh_out_recv dh_out_send new_dhs_pub
        (by rw [← Tacenta.SessionUnitT3.matchesHeader_eta]; exact hone)
        (by simpa [ratchetAbs, Tacenta.SessionUnitT3.max_skipped_store_agrees,
...
theorem spqr_agrees_for (hkr : Tacenta.SessionUnitSpqrT3.SpqrHkdfAgrees)
  ...
  have counter : ∀ s : tacenta_spqr.State,
    ...
    exact h (Tacenta.SessionUnitSpqrT3.chainsEntryOf p) (List.mem_map.2 ⟨p, hp, rfl⟩)
      (Tacenta.SessionUnitSpqrT3.chainOf ch)
      (by rcases hch with hch | hch <;>
            simp [Tacenta.SessionUnitSpqrT3.chainsEntryOf, Tacenta.SessionUnitSpqrT3.chainsOf, hch])
  unfold SpqrAgreesFor
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro s
    exact ⟨s, Tacenta.SessionUnitTripleT1.spqr_state_clone_id s, rfl⟩
  · intro s
  ...
  · intro s re out n hepoch hroom hcb hsb hnewb hskip hone hcounter
    obtain ⟨r, hr, hpost⟩ := Std.WP.spec_imp_exists
      (Tacenta.SessionUnitSpqrT3.receive_refines hkr hz96 hz64 hret happ hrm hzs hopt
        ...
        (fun p hp => hcb _ (List.mem_map.2 ⟨p, hp, rfl⟩))
        (fun sk hsk => hsb _ (List.mem_map.2 ⟨sk, hsk, rfl⟩))
        hnewb (by simpa [spqrAbs, Tacenta.SessionUnitSpqrT3.max_skip_agrees] using hskip)
        hone (counter s hcounter))
    refine ⟨r, hr, ?_⟩
    have hmap : out.map spqrOutputOf = out.map Tacenta.SessionUnitSpqrT3.outputOf := rfl
    rw [hmap]
    generalize Model.SparseRatchet.receive (spqrAbs s) re.val
      (out.map Tacenta.SessionUnitSpqrT3.outputOf) n.val = res at hpost ⊢
...
end Tacenta.SessionUnitTripleT3
