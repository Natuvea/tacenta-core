...
each crate independently, so braid's opaque `tacenta_session_unit.tacenta_erasure.Encoder` and
...
namespace Tacenta.SessionUnitBraidT3
...
crate its own `tacenta_session_unit.tacenta_kdf.hkdf_sha256`/`hmac_sha256` axioms (the counting
...
def BraidHkdfAgrees : Prop :=
  ∀ (N : Usize) (salt ikm info : Slice Std.U8), N.val ≤ 8160 →
    ∃ r, tacenta_session_unit.tacenta_kdf.hkdf_sha256 N salt ikm info = ok r ∧
      keyOf r = Model.Kdf.hkdf (sliceOf salt) (sliceOf ikm) (sliceOf info) N.val

def BraidHmacAgrees : Prop :=
  ∀ (key data : Slice Std.U8),
    ∃ r, tacenta_session_unit.tacenta_kdf.hmac_sha256 key data = ok r ∧
      keyOf r = Model.Kdf.hmac (sliceOf key) (sliceOf data)

theorem BraidHkdfAgrees.total (h : BraidHkdfAgrees) : Tacenta.SessionUnitBraidT1.HkdfSha256Total :=
  fun N a b c hN => let ⟨r, hr, _⟩ := h N a b c hN; ⟨r, hr⟩

theorem BraidHmacAgrees.total (h : BraidHmacAgrees) : Tacenta.SessionUnitBraidT1.HmacSha256Total :=
...
def KemLenAgrees (K : Model.Braid.Kem) : Prop :=
  (∃ v, tacenta_session_unit.tacenta_kem.HEADER_LEN = ok v ∧ v.val = Model.Braid.headerSize) ∧
  (∃ v, tacenta_session_unit.tacenta_kem.EK_VECTOR_LEN = ok v ∧ v.val = K.ekSize) ∧
  (∃ v, tacenta_session_unit.tacenta_kem.CT1_LEN = ok v ∧ v.val = K.ct1Size) ∧
  (∃ v, tacenta_session_unit.tacenta_kem.CT2_LEN = ok v ∧ v.val = K.ct2Size)
...
def KemAgreesFor (K : Model.Braid.Kem) : Prop :=
  (∀ {R : Type} (rc : tacenta_session_unit.rand_core_1.RngCore R) (crc : tacenta_session_unit.rand_core_1.CryptoRng R) (rng : R),
      ...
      ∃ kp rng' rand,
        ...
        (∃ h, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.header kp = ok h ∧
          vecOf h = (K.keyGen rand).2.1 ++ K.hashEk (K.keyGen rand).2.1 (K.keyGen rand).2.2) ∧
        (∃ v, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.ek_vector kp = ok v ∧
          vecOf v = (K.keyGen rand).2.2) ∧
        (∀ ct1 ct2 : Slice Std.U8, ct1.length = K.ct1Size → ct2.length = K.ct2Size →
          ∃ raw : Array Std.U8 32#usize,
            tacenta_session_unit.tacenta_kem.IncrementalKeyPair.decapsulate kp ct1 ct2 =
              ok (core.result.Result.Ok raw) ∧
            keyOf raw = K.decaps (K.keyGen rand).1 (sliceOf ct1) (sliceOf ct2))) ∧
    (∀ {R : Type} (rc : tacenta_session_unit.rand_core_1.RngCore R) (crc : tacenta_session_unit.rand_core_1.CryptoRng R)
      ...
      ∃ es ct1raw ssraw rng' rand',
        tacenta_session_unit.tacenta_kem.encapsulate1 rc crc header rng =
        ...
        (∀ ekSeed hek : Bytes, ekSeed.length = 32 → sliceOf header = ekSeed ++ hek →
          ...
          (∀ ekVector : Slice Std.U8, ekVector.length = K.ekSize →
            ∃ ct2raw,
              tacenta_session_unit.tacenta_kem.encapsulate2 es ekVector = ok (core.result.Result.Ok ct2raw) ∧
              vecOf ct2raw =
...
def ValidateEkAgrees (K : Model.Braid.Kem) : Prop :=
  ∀ (header ekVector : Slice Std.U8) (ekSeed hek : Bytes),
    ...
    ∃ r, tacenta_session_unit.tacenta_kem.validate_ek header ekVector = ok r ∧
...
def KemCloneAgrees : Prop :=
  (∀ kp kp' : tacenta_session_unit.tacenta_kem.IncrementalKeyPair,
    tacenta_session_unit.tacenta_kem.IncrementalKeyPair.Insts.CoreCloneClone.clone kp = ok kp' →
    (∀ h, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.header kp = ok h →
      tacenta_session_unit.tacenta_kem.IncrementalKeyPair.header kp' = ok h) ∧
    (∀ v, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.ek_vector kp = ok v →
      tacenta_session_unit.tacenta_kem.IncrementalKeyPair.ek_vector kp' = ok v) ∧
    (∀ (ct1 ct2 : Slice Std.U8) raw,
      tacenta_session_unit.tacenta_kem.IncrementalKeyPair.decapsulate kp ct1 ct2 =
        ok (core.result.Result.Ok raw) →
      tacenta_session_unit.tacenta_kem.IncrementalKeyPair.decapsulate kp' ct1 ct2 =
        ok (core.result.Result.Ok raw))) ∧
  (∀ es es' : tacenta_session_unit.tacenta_kem.EncapsState,
    tacenta_session_unit.tacenta_kem.EncapsState.Insts.CoreCloneClone.clone es = ok es' →
    ∀ (ekVector : Slice Std.U8) ct2, tacenta_session_unit.tacenta_kem.encapsulate2 es ekVector =
      ok (core.result.Result.Ok ct2) →
      tacenta_session_unit.tacenta_kem.encapsulate2 es' ekVector = ok (core.result.Result.Ok ct2))
...
def EncoderSim : Nat → tacenta_session_unit.tacenta_erasure.Encoder → Model.Braid.Encoder → Prop
  | 0, _, _ => True
  | n + 1, real, model =>
    ∃ chunk real', tacenta_session_unit.tacenta_erasure.Encoder.next_chunk real = ok (some chunk, real') ∧
      chunk.index.val = model.nextChunk.1.index ∧
        EncoderSim n real' model.nextChunk.2

/-- `chunk` is what `enc` emits after `i` further steps. -/
def NthChunk : Nat → tacenta_session_unit.tacenta_erasure.Encoder → tacenta_session_unit.tacenta_erasure.Chunk → Prop
  | 0, enc, chunk => ∃ enc', tacenta_session_unit.tacenta_erasure.Encoder.next_chunk enc = ok (some chunk, enc')
  | i + 1, enc, chunk =>
    ∃ c enc', tacenta_session_unit.tacenta_erasure.Encoder.next_chunk enc = ok (some c, enc') ∧ NthChunk i enc' chunk

/-- `real` is `enc0` advanced `k` steps. -/
def Stepped : Nat → tacenta_session_unit.tacenta_erasure.Encoder → tacenta_session_unit.tacenta_erasure.Encoder → Prop
  | 0, enc0, real => real = enc0
  | k + 1, enc0, real =>
    ∃ c enc', tacenta_session_unit.tacenta_erasure.Encoder.next_chunk enc0 = ok (some c, enc') ∧ Stepped k enc' real
...
def CodewordOf (m : Bytes) (chunk : tacenta_session_unit.tacenta_erasure.Chunk) : Prop :=
  ∃ (s : Slice Std.U8) (enc0 : tacenta_session_unit.tacenta_erasure.Encoder), sliceOf s = m ∧
    tacenta_session_unit.tacenta_erasure.Encoder.new s = ok enc0 ∧ NthChunk chunk.index.val enc0 chunk

def DecoderSim (m : Bytes) : Nat → tacenta_session_unit.tacenta_erasure.Decoder → Model.Braid.Decoder → Prop
  | 0, _, _ => True
  | n + 1, real, model =>
    (∀ (chunk : tacenta_session_unit.tacenta_erasure.Chunk) (advanced : Bool) (real' : tacenta_session_unit.tacenta_erasure.Decoder),
      CodewordOf m chunk →
      tacenta_session_unit.tacenta_erasure.Decoder.add_chunk real chunk = ok (advanced, real') →
      DecoderSim m n real' (model.addChunk ⟨m, chunk.index.val⟩)) ∧
    (∀ msg, tacenta_session_unit.tacenta_erasure.Decoder.message real = ok msg →
...
def EncoderRefines (real : tacenta_session_unit.tacenta_erasure.Encoder) (model : Model.Braid.Encoder) : Prop :=
  (∃ (s : Slice Std.U8) (enc0 : tacenta_session_unit.tacenta_erasure.Encoder), sliceOf s = model.source ∧
      tacenta_session_unit.tacenta_erasure.Encoder.new s = ok enc0 ∧ Stepped model.next enc0 real) ∧
...
def DecoderRefines (real : tacenta_session_unit.tacenta_erasure.Decoder) (model : Model.Braid.Decoder) : Prop :=
  ∀ m : Bytes, m.length = model.size → (∀ c ∈ model.chunks, c.source = m) →
    ∀ n, DecoderSim m n real model

def ErasureAgrees : Prop :=
  (∀ (s : Slice Std.U8), ∃ real, tacenta_session_unit.tacenta_erasure.Encoder.new s = ok real ∧
    EncoderRefines real (Model.Braid.encode (sliceOf s))) ∧
  (∀ (n : Usize), ∃ real, tacenta_session_unit.tacenta_erasure.Decoder.new n = ok real ∧
...
def ErasureCloneAgrees : Prop :=
  (∀ (real real' : tacenta_session_unit.tacenta_erasure.Encoder),
    tacenta_session_unit.tacenta_erasure.Encoder.Insts.CoreCloneClone.clone real = ok real' → real' = real) ∧
  (∀ (real real' : tacenta_session_unit.tacenta_erasure.Decoder),
    tacenta_session_unit.tacenta_erasure.Decoder.Insts.CoreCloneClone.clone real = ok real' → real' = real)
...
def KeyPairRefines (K : Model.Braid.Kem) (ikp : tacenta_session_unit.tacenta_kem.IncrementalKeyPair)
    (dk : Bytes) : Prop :=
  ∀ ct1 ct2 : Slice Std.U8, ct1.length = K.ct1Size → ct2.length = K.ct2Size →
    ∃ raw, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.decapsulate ikp ct1 ct2 =
      ok (core.result.Result.Ok raw) ∧ keyOf raw = K.decaps dk (sliceOf ct1) (sliceOf ct2)

def FreshKeyPairRefines (K : Model.Braid.Kem) (ikp : tacenta_session_unit.tacenta_kem.IncrementalKeyPair)
    (dk ekVector : Bytes) : Prop :=
  KeyPairRefines K ikp dk ∧
  (∃ v, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.ek_vector ikp = ok v ∧ vecOf v = ekVector) ∧
  (∃ h ekSeed, tacenta_session_unit.tacenta_kem.IncrementalKeyPair.header ikp = ok h ∧
    vecOf h = ekSeed ++ K.hashEk ekSeed ekVector)

def EncapsRefines (K : Model.Braid.Kem) (es : tacenta_session_unit.tacenta_kem.EncapsState)
    (ekSeed encapsSecret : Bytes) : Prop :=
  ∀ ekVector : Slice Std.U8, ekVector.length = K.ekSize →
    ∃ ct2raw, tacenta_session_unit.tacenta_kem.encapsulate2 es ekVector = ok (core.result.Result.Ok ct2raw) ∧
      vecOf ct2raw = K.encaps2 encapsSecret ekSeed (sliceOf ekVector)

theorem Stepped.succ {k : Nat} {enc0 real real' : tacenta_session_unit.tacenta_erasure.Encoder}
    {c : tacenta_session_unit.tacenta_erasure.Chunk} (hs : Stepped k enc0 real)
    (h : tacenta_session_unit.tacenta_erasure.Encoder.next_chunk real = ok (some c, real')) :
...
theorem Stepped.nthChunk {k : Nat} {enc0 real real' : tacenta_session_unit.tacenta_erasure.Encoder}
    {c : tacenta_session_unit.tacenta_erasure.Chunk} (hs : Stepped k enc0 real)
    (h : tacenta_session_unit.tacenta_erasure.Encoder.next_chunk real = ok (some c, real')) :
...
theorem EncoderRefines.step {real : tacenta_session_unit.tacenta_erasure.Encoder} {model : Model.Braid.Encoder}
    (hr : EncoderRefines real model) (hlive : model.next < 65536)
    {chunk : tacenta_session_unit.tacenta_erasure.Chunk} {real' : tacenta_session_unit.tacenta_erasure.Encoder}
    (h : tacenta_session_unit.tacenta_erasure.Encoder.next_chunk real = ok (some chunk, real')) :
...
theorem DecoderRefines.add {real real' : tacenta_session_unit.tacenta_erasure.Decoder}
    {model : Model.Braid.Decoder} (hr : DecoderRefines real model)
    {chunk : tacenta_session_unit.tacenta_erasure.Chunk} {advanced : Bool} {mc : Model.Braid.Chunk}
    (hsame : ∀ c ∈ model.chunks, c.source = mc.source)
    (hcw : CodewordOf mc.source chunk) (hidx : chunk.index.val = mc.index)
    (hadd : tacenta_session_unit.tacenta_erasure.Decoder.add_chunk real chunk = ok (advanced, real')) :
...
theorem DecoderRefines.add_message {real real' : tacenta_session_unit.tacenta_erasure.Decoder}
    {model : Model.Braid.Decoder} (hr : DecoderRefines real model)
    {chunk : tacenta_session_unit.tacenta_erasure.Chunk} {advanced : Bool} {mc : Model.Braid.Chunk}
    ...
    (hmsg : tacenta_session_unit.tacenta_erasure.Decoder.message real' = ok omsg) :
...
theorem KeyPairRefines.clone (hcl : KemCloneAgrees) {K : Model.Braid.Kem}
    {ikp ikp' : tacenta_session_unit.tacenta_kem.IncrementalKeyPair} {dk : Bytes}
    (h : tacenta_session_unit.tacenta_kem.IncrementalKeyPair.Insts.CoreCloneClone.clone ikp = ok ikp')
...
theorem FreshKeyPairRefines.clone (hcl : KemCloneAgrees) {K : Model.Braid.Kem}
    {ikp ikp' : tacenta_session_unit.tacenta_kem.IncrementalKeyPair} {dk ekVector : Bytes}
    (h : tacenta_session_unit.tacenta_kem.IncrementalKeyPair.Insts.CoreCloneClone.clone ikp = ok ikp')
...
theorem EncapsRefines.clone (hcl : KemCloneAgrees) {K : Model.Braid.Kem}
    {es es' : tacenta_session_unit.tacenta_kem.EncapsState} {ekSeed encapsSecret : Bytes}
    (h : tacenta_session_unit.tacenta_kem.EncapsState.Insts.CoreCloneClone.clone es = ok es')
...
theorem EncoderRefines.clone (hcl : ErasureCloneAgrees) {real real' : tacenta_session_unit.tacenta_erasure.Encoder}
    {model : Model.Braid.Encoder}
    (h : tacenta_session_unit.tacenta_erasure.Encoder.Insts.CoreCloneClone.clone real = ok real')
    (hr : EncoderRefines real model) : EncoderRefines real' model := by
  rw [hcl.1 real real' h]; exact hr

theorem DecoderRefines.clone (hcl : ErasureCloneAgrees) {real real' : tacenta_session_unit.tacenta_erasure.Decoder}
    {model : Model.Braid.Decoder}
    (h : tacenta_session_unit.tacenta_erasure.Decoder.Insts.CoreCloneClone.clone real = ok real')
...
theorem Auth.update_refines (hkdf : BraidHkdfAgrees) (hz : Tacenta.SessionUnitBraidT1.ZeroizingArrayRoundTrip)
  ...
  step*
  step with Tacenta.SessionUnitBraidT1.zeroizing_deref_spec ‹_›
  step*
...
theorem Auth.init_refines (hkdf : BraidHkdfAgrees) (hz : Tacenta.SessionUnitBraidT1.ZeroizingArrayRoundTrip)
...
theorem State.clone_refines (hkcl : KemCloneAgrees) (hecl : ErasureCloneAgrees)
    (henc : Tacenta.SessionUnitBraidT1.EncoderCloneTotal) (hdec : Tacenta.SessionUnitBraidT1.DecoderCloneTotal)
  ...
  case Ct1Received.ct1Received =>
    ...
    obtain ⟨ikp', hikp'⟩ := hkp ‹_›
    simp only [hikp']
    step with Tacenta.SessionUnitBraidT1.vecU8_clone_no_panic
    obtain ⟨enc', henc'⟩ := henc ‹_›
    simp only [henc']
    have := KeyPairRefines.clone hkcl hikp' hkpr
    have := EncoderRefines.clone hecl henc' her
...
def MsgTypeRefines : tacenta_session_unit.tacenta_braid.MsgType → Model.Braid.MsgType → Prop
  | .None, .none => True
  | .Hdr, .hdr => True
  | .Ek, .ek => True
...
theorem MsgTypeRefines.eq_agrees {a : tacenta_session_unit.tacenta_braid.MsgType} {ma : Model.Braid.MsgType}
    (ha : MsgTypeRefines a ma) {b : tacenta_session_unit.tacenta_braid.MsgType} {mb : Model.Braid.MsgType}
...
def MsgRefines (real : tacenta_session_unit.tacenta_braid.Msg) (model : Model.Braid.Msg) : Prop :=
...
def OutputRefines (real : tacenta_session_unit.tacenta_braid.Output) (model : Model.Braid.Output) : Prop :=
  real.key_epoch.val = model.keyEpoch ∧ keyOf real.key = model.key

def OptionOutputRefines :
    Option tacenta_session_unit.tacenta_braid.Output → Option Model.Braid.Output → Prop
  | none, none => True
...
theorem step_send_refines (hka : KemAgreesFor K) (hea : ErasureAgrees)
    ...
    (hhdr : Tacenta.SessionUnitBraidT1.KeyPairHeaderTotal) (_hekv : Tacenta.SessionUnitBraidT1.KeyPairEkVectorTotal)
    (_hct1len : Tacenta.SessionUnitBraidT1.Ct1LenTotal) (_hct2len : Tacenta.SessionUnitBraidT1.Ct2LenTotal)
  ...
  case HeaderReceived.headerReceived =>
    ...
    step with Tacenta.SessionUnitBraidT1.zeroizing_new_spec hz
    step with Tacenta.SessionUnitBraidT1.array_zeroize_spec hzz
    step with Tacenta.SessionUnitBraidT1.zeroizing_deref_spec ‹_›
    step with Tacenta.SessionUnitBraidT1.index_full_spec hrf
...
theorem finish_encaps_refines (hmac : BraidHmacAgrees) (hea : ErasureAgrees)
    {K : Model.Braid.Kem} (epoch : Std.U64) (auth : Auth) (encaps : tacenta_session_unit.tacenta_kem.EncapsState)
...
theorem hdr_decoder_refines (hea : ErasureAgrees)
    (hheaderlen : ∃ v : Usize, tacenta_session_unit.tacenta_kem.HEADER_LEN = ok v ∧ v.val = Model.Braid.headerSize) :
    hdr_decoder ⦃ fun r =>
...
theorem step_receive_refines (hka : KemAgreesFor K) (hea : ErasureAgrees)
    ...
    (hdadd : Tacenta.SessionUnitBraidT1.DecoderAddChunkTotal)
    (hdmsg : Tacenta.SessionUnitBraidT1.DecoderMessageTotal)
    (_hct1lenB : Tacenta.SessionUnitBraidT1.Ct1LenTotal) (hct2lenB : Tacenta.SessionUnitBraidT1.Ct2LenTotal)
  ...
  case Ct2Sampled.ct2Sampled =>
    ...
    have hepoch1 : epoch1.val + 1 < Std.U64.max := by
      simpa [Tacenta.SessionUnitBraidT1.State.epoch_val] using hepoch
      ...
      · have htyM : modelMsg.type = Model.Braid.MsgType.ekCt1Ack := b_post.mp hty
        ...
        have hlen : ct1.length +
          ...
          have hct1bound : ct1.length ≤ 4096 := by
            simpa [Tacenta.SessionUnitBraidT1.State.ct1_bounded] using hct1b
          scalar_tac
        ...
        · rename_i hdm2
          ...
          · obtain ⟨framedM, hMsome⟩ :
            ...
            · -- Transition 5's `checked_add` came back `none`: unreachable below
              ...
              exfalso
              rw [ho1] at o1_post
              simp only [Tacenta.SessionUnitBraidT1.State.epoch_val] at hepoch
              simp at o1_post
              scalar_tac
            ...
            · -- The successor is `some` and is not the reserved value: the two
              ...
              · rw [if_pos hb1]
                ...
                have hceilM : epoch' + 1 < Model.Braid.u64Max :=
                  model_ceiling he (by simpa [Tacenta.SessionUnitBraidT1.State.epoch_val] using hepoch)
                unfold Model.Braid.receive
...
theorem State.clone_bounds_refines {K : Model.Braid.Kem} {self s : State}
    ...
    (Tacenta.SessionUnitBraidT1.State.epoch_val s).val = (Tacenta.SessionUnitBraidT1.State.epoch_val self).val ∧
    (Tacenta.SessionUnitBraidT1.State.ct1_bounded self → Tacenta.SessionUnitBraidT1.State.ct1_bounded s) := by
  ...
  case KeysUnsampled.keysUnsampled =>
    cases s <;> simp_all [StateRefines, Tacenta.SessionUnitBraidT1.State.epoch_val,
  ...
  case Ct1Received.ct1Received =>
    ...
    refine ⟨by simp [Tacenta.SessionUnitBraidT1.State.epoch_val, he, heS], ?_⟩
    simp only [Tacenta.SessionUnitBraidT1.State.ct1_bounded]
    intro h
...
theorem Braid.receive_refines (hka : KemAgreesFor K) (hea : ErasureAgrees)
  ...
  · obtain ⟨mo, hMsome⟩ : ∃ mo, (Model.Braid.receive K model modelMsg).2.1 = some mo := by
    ...
    rw [hom] at hout
    rw [hMsome] at hout
    step with hopt Output.Insts.CoreCloneClone (some o) (fun x _ => Tacenta.SessionUnitBraidT1.Output.clone_no_panic x)
...
/--
info: 'Tacenta.SessionUnitBraidT3.Braid.send_refines' depends on axioms: [propext,
 Classical.choice,
 Quot.sound,
...
-/
#guard_msgs in
#print axioms Tacenta.SessionUnitBraidT3.Braid.send_refines

/--
info: 'Tacenta.SessionUnitBraidT3.Braid.receive_refines' depends on axioms: [propext,
 Classical.choice,
 Quot.sound,
 tacenta_session_unit.tacenta_kdf.hkdf_sha256,
 tacenta_session_unit.tacenta_kdf.hmac_sha256,
...
-/
#guard_msgs in
#print axioms Tacenta.SessionUnitBraidT3.Braid.receive_refines

end Tacenta.SessionUnitBraidT3
