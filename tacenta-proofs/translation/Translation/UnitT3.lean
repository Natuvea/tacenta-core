...
-- Translation/T3.lean. Do not edit: `port-unit-proofs.sh --check`
-- regenerates this file and fails on any difference.
--
-- This is T3.lean restated about the three-leaf translation unit, whose
-- constants are different constants from the ones T3.lean is about. Only the
...
open Tacenta.UnitT1 (DerivedKeysModel)
...
attribute [-step] Tacenta.UnitT1.array_eq_total
...
attribute [-step] Tacenta.UnitT1.array_ne_total

@[step]
theorem array_ne_val {N : Usize} (a b : Array Std.U8 N) :
    core.array.equality.PartialEqArray.ne core.cmp.PartialEqU8 a b ⦃ fun r =>
      r = true ↔ a ≠ b ⦄ := by
  obtain ⟨v, hv⟩ := (Tacenta.UnitT1.noPanic_iff _).mp (Tacenta.UnitT1.array_eq_total a b)
...
theorem HmacAgrees.total (h : HmacAgrees) : Tacenta.UnitT1.HmacTotal :=
  fun key data => let ⟨r, hr, _⟩ := h key data; ⟨r, hr⟩

theorem HkdfAgrees.total (h : HkdfAgrees) : Tacenta.UnitT1.HkdfTotal :=
...
attribute [-step] Tacenta.UnitT1.kdf_ck_step
...
theorem kdf_rk_refines (h : HkdfAgrees) (hz : ZeroizingRoundTrips)
  ...
  have hht : Tacenta.UnitT1.HkdfTotal := h.total
  unfold kdf_rk
  step*
  simp only [out_post]
...
theorem try_skipped_refines (hrm : Tacenta.UnitT1.VecRemoveTotal)
  ...
  obtain ⟨rr, hrr⟩ := (Tacenta.UnitT1.noPanic_iff _).mp
    (Tacenta.UnitT1.try_skipped_loop_no_panic hrm s hdr 0#usize)
...
attribute [-step] Tacenta.UnitT1.age_store_spec
...
theorem receive_tail_refines (h : HmacAgrees) (hrm : Tacenta.UnitT1.VecRemoveTotal)
    ...
    ⦃ fun r => ∀ mk, r.1 = core.result.Result.Ok mk →
      ∃ m',
        ...
        ∧ StateR r.2 m' ⦄ := by
  have hht : Tacenta.UnitT1.HmacTotal := h.total
  obtain ⟨rk, hrk⟩ := (Tacenta.UnitT1.noPanic_iff _).mp
...
attribute [-step] Tacenta.UnitT1.skip_message_keys_bound Tacenta.UnitT1.dh_ratchet_spec
