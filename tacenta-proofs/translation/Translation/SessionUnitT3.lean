...
-- Translation/UnitT3.lean. Do not edit: `port-session-unit-proofs.sh --check`
...
namespace Tacenta.SessionUnitT3

open tacenta_session_unit tacenta_session_unit.tacenta_ratchet
...
attribute [-step] Tacenta.SessionUnitT1.array_eq_total
...
attribute [-step] Tacenta.SessionUnitT1.array_ne_total

@[step]
theorem array_ne_val {N : Usize} (a b : Array Std.U8 N) :
    core.array.equality.PartialEqArray.ne core.cmp.PartialEqU8 a b ⦃ fun r =>
      r = true ↔ a ≠ b ⦄ := by
  obtain ⟨v, hv⟩ := (Tacenta.SessionUnitT1.noPanic_iff _).mp (Tacenta.SessionUnitT1.array_eq_total a b)
...
theorem HmacAgrees.total (h : HmacAgrees) : Tacenta.SessionUnitT1.HmacTotal :=
  fun key data => let ⟨r, hr, _⟩ := h key data; ⟨r, hr⟩

theorem HkdfAgrees.total (h : HkdfAgrees) : Tacenta.SessionUnitT1.HkdfTotal :=
...
attribute [-step] Tacenta.SessionUnitT1.kdf_ck_step
...
theorem kdf_rk_refines (h : HkdfAgrees) (hz : ZeroizingRoundTrips)
  ...
  have hht : Tacenta.SessionUnitT1.HkdfTotal := h.total
  unfold kdf_rk
  step*
  simp only [out_post]
...
theorem purge_chain_range_loop_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
...
@[step]
theorem purge_chain_range_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
...
theorem age_store_loop_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    (now : Std.U32)
    (target : List (Model.State.Key × Nat × Nat × Model.State.Key))
    (skipped : alloc.vec.Vec SkippedKey) (i : Usize)
    (hinv : (skipped.val.take i.val).map skippedOf
...
theorem age_store_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    (s : State) (m : Model.State.State) (hR : StateR s m)
...
attribute [-step] Tacenta.SessionUnitT1.skip_message_keys_loop0_grows
attribute [-step] Tacenta.SessionUnitT1.skip_message_keys_loop0_bound

theorem skip_message_keys_refines (h : HmacAgrees)
    (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal) [DerivedKeysModel] (s : State)
...
theorem try_skipped_loop_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
...
theorem try_skipped_loop_fields (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
...
theorem try_skipped_refines (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    (s : State) (m : Model.State.State) (hR : StateR s m)
    (hdr : Header) (mh : Model.State.Header) (hH : HeaderR hdr mh)
  ...
  obtain ⟨rr, hrr⟩ := (Tacenta.SessionUnitT1.noPanic_iff _).mp
    (Tacenta.SessionUnitT1.try_skipped_loop_no_panic hrm s hdr 0#usize)
...
attribute [-step] Tacenta.SessionUnitT1.age_store_spec
...
theorem receive_tail_refines (h : HmacAgrees) (hrm : Tacenta.SessionUnitT1.RemoveSkippedAtTotal)
    ...
    ⦃ fun r => ∀ mk, r.1 = core.result.Result.Ok mk →
      ∃ m',
        ...
        ∧ StateR r.2 m' ⦄ := by
  have hht : Tacenta.SessionUnitT1.HmacTotal := h.total
  obtain ⟨rk, hrk⟩ := (Tacenta.SessionUnitT1.noPanic_iff _).mp
...
attribute [-step] Tacenta.SessionUnitT1.skip_message_keys_room Tacenta.SessionUnitT1.dh_ratchet_spec
