...
`tacenta-ratchet` is, and this file is its counterpart to `Translation.SessionUnitT1`: every
...
namespace Tacenta.SessionUnitSessionT1

open Aeneas Aeneas.Std Result
open tacenta_session_unit.tacenta_session

/-- The shape shared with the ratchet's tier: a call that does not fail. -/
abbrev NoPanic {α : Type} (e : Result α) : Prop := Tacenta.SessionUnitT1.NoPanic e
...
class ZeroizingModel where
  ...
  contents : tacenta_session_unit.zeroize.Zeroizing (alloc.vec.Vec U8) → alloc.vec.Vec U8
  /-- Wrapping stores what it is given. -/
  new : ∀ (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8)) (v : alloc.vec.Vec U8),
    tacenta_session_unit.zeroize.Zeroizing.new inst v ⦃ fun z => contents z = v ⦄
  /-- Reading returns the contents. -/
  deref : ∀ (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8))
      (z : tacenta_session_unit.zeroize.Zeroizing (alloc.vec.Vec U8)),
    tacenta_session_unit.zeroize.Zeroizing.Insts.CoreOpsDerefDeref.deref inst z ⦃ fun v => v = contents z ⦄
  /-- Reading mutably returns the contents and a way to replace them. -/
  deref_mut : ∀ (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8))
      (z : tacenta_session_unit.zeroize.Zeroizing (alloc.vec.Vec U8)),
    tacenta_session_unit.zeroize.Zeroizing.Insts.CoreOpsDerefDerefMut.deref_mut inst z ⦃ fun p =>
...
@[step]
theorem zeroizing_new_step (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8))
    (v : alloc.vec.Vec U8) :
    tacenta_session_unit.zeroize.Zeroizing.new inst v ⦃ fun z => ZeroizingModel.contents z = v ⦄ :=
  ZeroizingModel.new inst v

@[step]
theorem zeroizing_deref_step (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8))
    (z : tacenta_session_unit.zeroize.Zeroizing (alloc.vec.Vec U8)) :
    tacenta_session_unit.zeroize.Zeroizing.Insts.CoreOpsDerefDeref.deref inst z
      ⦃ fun v => v = ZeroizingModel.contents z ⦄ := ZeroizingModel.deref inst z

@[step]
theorem zeroizing_deref_mut_step (inst : tacenta_session_unit.zeroize.Zeroize (alloc.vec.Vec U8))
    (z : tacenta_session_unit.zeroize.Zeroizing (alloc.vec.Vec U8)) :
    tacenta_session_unit.zeroize.Zeroizing.Insts.CoreOpsDerefDerefMut.deref_mut inst z ⦃ fun p =>
...
theorem hkdf_step (hk : HkdfTotal) (N : Usize) (salt ikm info : Slice U8)
    (hN : N.val ≤ 8160) :
    tacenta_session_unit.tacenta_kdf.hkdf_sha256 N salt ikm info ⦃ fun _ => True ⦄ := by
...
/--
info: 'Tacenta.SessionUnitSessionT1.shared_secret_no_panic' depends on axioms: [propext,
 Classical.choice,
 Quot.sound,
...
-/
#guard_msgs in
#print axioms Tacenta.SessionUnitSessionT1.shared_secret_no_panic
...
end Tacenta.SessionUnitSessionT1
