...
-- Translation/UnitTripleT1.lean. Do not edit: `port-session-unit-proofs.sh --check`
...
(`Translation/SessionUnitT1.lean`, `Translation/SessionUnitSpqrT1.lean`) are about exactly
...
`scripts/port-unit-proofs.sh` derives `SessionUnitT1.lean` and `SessionUnitSpqrT1.lean` from
...
`zeroize.`). Under `open tacenta_session_unit` those prefixes
...
* about twenty `@[step]` rules from the two leaf files are newly in scope, so
  ...
  `SessionUnitT1.hkdf_step` now fires on its own;
...
* three of `TripleT1.lean`'s boundary copies collapsed into the leaves'
  (`HkdfSha256Total` and `ZeroizingTotal` are `rfl`-equal to `SessionUnitT1.HkdfTotal`
  and `SessionUnitT1.ZeroizingTotal`; its `ZeroizeTotal` is a narrowing of
  `SessionUnitSpqrT1.ZeroizeTotal` at one instance, which has to be written out at
  every use), and its two `@[step]` `zeroize.Zeroizing` helpers are duplicates
  of `SessionUnitT1`'s, so none of the five is redeclared. Two of the three collapses
  are exact and the third is not: the two constructor theorems below take
  `SessionUnitSpqrT1.ZeroizeTotal`, which quantifies over the element type, the width
...
`SessionUnitSpqrT1.KdfRkTotal`: a translated function bottoming out in the opaque
...
and the `info` builder, exactly as `SessionUnitSpqrT1.KdfRkTotal` does.
...
`SessionUnitSpqrT1.OptionCloneTotal`, or a `Vec` whose elements clone as the
...
namespace Tacenta.SessionUnitTripleT1

open tacenta_session_unit tacenta_session_unit.tacenta_triple
...
theorem option_clone_id (hopt : SessionUnitSpqrT1.OptionCloneTotal) {T : Type}
...
theorem option_byte_array_clone_id (hopt : SessionUnitSpqrT1.OptionCloneTotal) {N : Usize}
    (o : Option (Array U8 N)) :
...
theorem ratchet_state_clone_id (hopt : SessionUnitSpqrT1.OptionCloneTotal)
...
theorem triple_state_clone_id (hopt : SessionUnitSpqrT1.OptionCloneTotal) (s : State) :
...
theorem ratchetStateCloneTotal (hopt : SessionUnitSpqrT1.OptionCloneTotal) :
...
theorem ratchetInitSenderTotal (hk : SessionUnitT1.HkdfTotal) (hzw : SessionUnitT1.ZeroizingTotal) :
    RatchetInitSenderTotal := by
  intro ec our_pub peer_pub dh_out labels
  obtain ⟨⟨rk, cks⟩, hr⟩ :=
    (SessionUnitT1.noPanic_iff _).1 (SessionUnitT1.kdf_rk_no_panic hk hzw ec dh_out labels)
...
theorem ratchetSendTotal (h : SessionUnitT1.HmacTotal) : RatchetSendTotal :=
  fun s => SessionUnitT1.send_no_panic h s
...
theorem ratchetReceiveTotal (h : SessionUnitT1.HmacTotal) (hk : SessionUnitT1.HkdfTotal)
    (hzw : SessionUnitT1.ZeroizingTotal) (hrm : SessionUnitT1.RemoveSkippedAtTotal)
    [SessionUnitT1.DerivedKeysModel] : RatchetReceiveTotal := by
  intro s hdr a b c hs
  exact (SessionUnitT1.noPanic_iff _).1 (SessionUnitT1.receive_no_panic h hk hzw hrm s hdr a b c hs)
...
theorem ratchetHeaderEqTotal : RatchetHeaderEqTotal := by
  ...
  split
  · split
    · exact (SessionUnitT1.noPanic_iff _).1 (SessionUnitT1.array_eq_total a.dh b.dh)
    · simp
  · simp
...
theorem spqrSendTotal (hret : SessionUnitSpqrT1.VecRetainTotal) (hrk : SessionUnitSpqrT1.KdfRkTotal)
    (hz : SessionUnitSpqrT1.ZeroizeTotal) (hck : SessionUnitSpqrT1.KdfCkTotal)
    (hopt : SessionUnitSpqrT1.OptionCloneTotal) : SpqrSendTotal := by
  intro s epoch out hroom
  exact (SessionUnitT1.noPanic_iff _).1
    (SessionUnitSpqrT1.send_no_panic hret hrk hz hck hopt s epoch out hroom)
...
theorem spqrReceiveTotal (hret : SessionUnitSpqrT1.VecRetainTotal) (hrk : SessionUnitSpqrT1.KdfRkTotal)
    (hz : SessionUnitSpqrT1.ZeroizeTotal) (hck : SessionUnitSpqrT1.KdfCkTotal)
    (hopt : SessionUnitSpqrT1.OptionCloneTotal) (hrm : SessionUnitSpqrT1.RemoveSkippedAtTotal)
    (happ : SessionUnitSpqrT1.VecAppendTotal) : SpqrReceiveTotal := by
  intro s epoch out n hroom hskiproom
  exact (SessionUnitT1.noPanic_iff _).1
    (SessionUnitSpqrT1.receive_no_panic hret hrk hz hck hopt hrm happ s epoch n out hroom hskiproom)
...
theorem split_secret_no_panic (hkdf : SessionUnitT1.HkdfTotal) (hzw : SessionUnitT1.ZeroizingTotal)
    (sk : Slice U8) : split_secret sk ⦃ fun _ => True ⦄ := by
  unfold split_secret
  step*
  all_goals (try simp_all [Slice.length])

theorem combine_no_panic (hkdf : SessionUnitT1.HkdfTotal) (mk_classical mk_pq : Array U8 32#usize) :
...
theorem State.clone_no_panic (hopt : SessionUnitSpqrT1.OptionCloneTotal) (self : State) :
...
theorem State.init_sender_no_panic (hss : SessionUnitT1.HkdfTotal) (hzw : SessionUnitT1.ZeroizingTotal)
    (hki : KdfInitTotal) (hz : SessionUnitSpqrT1.ZeroizeTotal) (sk : Slice U8)
...
theorem State.init_receiver_no_panic (hss : SessionUnitT1.HkdfTotal) (hzw : SessionUnitT1.ZeroizingTotal)
    (hki : KdfInitTotal) (hz : SessionUnitSpqrT1.ZeroizeTotal) (sk : Slice U8)
...
theorem State.clone_spec (hopt : SessionUnitSpqrT1.OptionCloneTotal) (self : State) :
...
instance argument because `SessionUnitSpqrT1.ZeroizeTotal` quantifies over it and the
stepping tactic has nothing to infer it from. -/
@[step]
theorem zeroize_step (hz : SessionUnitSpqrT1.ZeroizeTotal) {Z : Type} {N : Usize}
...
@[step]
theorem combine_step (hkdf : SessionUnitT1.HkdfTotal) (a b : Array U8 32#usize) :
...
theorem State.send_no_panic (hhmac : SessionUnitT1.HmacTotal) (hkdf : SessionUnitT1.HkdfTotal)
    (hz : SessionUnitSpqrT1.ZeroizeTotal) (hret : SessionUnitSpqrT1.VecRetainTotal)
    (hrk : SessionUnitSpqrT1.KdfRkTotal) (hck : SessionUnitSpqrT1.KdfCkTotal)
    (hopt : SessionUnitSpqrT1.OptionCloneTotal)
...
theorem State.receive_no_panic (hhmac : SessionUnitT1.HmacTotal) (hkdf : SessionUnitT1.HkdfTotal)
    (hzw : SessionUnitT1.ZeroizingTotal) [SessionUnitT1.DerivedKeysModel]
    (hz : SessionUnitSpqrT1.ZeroizeTotal) (hret : SessionUnitSpqrT1.VecRetainTotal)
    (hrk : SessionUnitSpqrT1.KdfRkTotal) (hck : SessionUnitSpqrT1.KdfCkTotal)
    (hopt : SessionUnitSpqrT1.OptionCloneTotal)
    (hrm_ratchet : SessionUnitT1.RemoveSkippedAtTotal)
    (hrm : SessionUnitSpqrT1.RemoveSkippedAtTotal) (happ : SessionUnitSpqrT1.VecAppendTotal)
