...
back from a byte string is not merely well-formed for the parser: it is one the
crate's operations could have built. Until now nothing in this tree said so.
...
This module closes that gap for `tacenta-ratchet` and `tacenta-spqr` in full,
and for `tacenta-braid` in the one clause its theorems need. Per crate it
gives three things, in a chain a reader can follow end to end:
...
* For the sparse ratchet, `SpqrT3.receive_refines`'s `hcb`, `hsb`, `hnewb` and
  `hcounter` are **not** consequences of the crate's `invariant`, and are not
  claimed here. A state with `epoch = u64::MAX - 1` and a chain at that epoch
  passes `invariant` and fails `hcb`; saying otherwise would be false. Its
  `hepoch`, `hroom`, `hskiproom` and `hone` are proved.
* For the Braid, only `ct1_bounded` is derived. The rest of the Rust
  ...
  `BraidT3.step_receive_refines`'s `hepoch` (`epoch < u64::MAX`) is not a
  clause the Rust `invariant` checks at all.
-/
...
`State::invariant` (ratchet/src/lib.rs) is four clauses: the store is at most
`MAX_SKIPPED_STORE`, the clock is below `u32::MAX`, no stored entry is ahead
of the clock and no two entries share `(dh, n)`, and the chains are present in
the order the operations open them. The first two are read straight off the
final `if` chain; the middle two are what the two nested index loops compute
into `store_ok`. -/
namespace Ratchet
...
Any byte string at all, hostile or corrupt: if `from_bytes` accepts it, the
state it hands back is one the operations could have built. -/
...
/-- `Inv` → `T3.receive_refines`'s `hroom`: the clock is below saturation, so
the refinement of `receive` covers this state. -/
...
/-- **Decoded state → `Inv` → `hone`, `hs`, `hroom` → `T3.receive_refines`.** A
`receive` on a state that came out of `from_bytes` refines the model's. The
relation to the model state is still a hypothesis: `from_bytes` says which
translated states are reachable, not which model state a caller means. -/
...
/-- `Inv` → `SpqrT3.receive_refines`'s `hepoch`. The current epoch is among the
chains, and every chain's epoch is strictly below its own saturating window
end, which is at most `u64::MAX`. -/
...
refinement also asks for `hcb`, `hsb`, `hnewb` and `hcounter`, and those are
not consequences of this crate's `invariant`: a state with
`epoch = u64::MAX - 1` and a chain at that epoch passes `invariant` and fails
`hcb`. Four of the refinement's premises (`hepoch`, `hroom`, `hskiproom`,
`hone`) are supplied above; the remaining four are open, and are recorded as
open in `CLAIMS.md`. -/
...
also asks for `hepoch` (`epoch < u64::MAX`), which the Rust `invariant` does
not check -- it checks `epoch >= 1` and nothing above. That premise stays with
the caller. -/
...
The ratchet's and the sparse ratchet's statements rest on Lean's three standard
axioms and on nothing else: no opaque primitive, and no assumption about our
own code. The Braid's pulls in the KEM constants and the erasure coders, which
are what the translated Braid calls and cannot see inside; that list is pinned
so a new one fails here rather than passing unnoticed. -/
