...
/-! A successful ratchet decoder result already carries the wire/model
    correspondence needed by every nonterminal branch.  Keep this inversion
    at one boundary: callers must provide the actual generated result and the
    model's exact `decodeDetailed` result, while the branch providers receive
    the recovered `CompositeRefines` relation rather than a second, unrelated
    header witness. -/
theorem decode_message_model_facts_of_nonrefusal
    (message : Slice Std.U8)
    (hnotbad : ∀ realReason,
      tacenta_wire.decode_message message ≠ ok (.Err realReason))
    (modelComposite : Model.CompositeHeader.Composite)
    (ciphertext : Bytes)
    (hmodelDecode : Model.CompositeHeader.decodeDetailed (sliceOf message) =
      .ok (modelComposite, ciphertext)) :
    ∃ decoded,
      tacenta_wire.decode_message message = ok (.Ok decoded) ∧
      vecOf decoded.ciphertext = ciphertext ∧
...
/-! Concrete evidence packages for the post-DH refusal leaves.  They carry
...
structure InitialRatchetTripleConcreteProviders
    ...
    (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
      (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
      (real := real) (model := model) message rng) where
  triple : ∀ (composite : Model.CompositeHeader.Composite) (ciphertext : Bytes)
...
/-! The Triple and AEAD providers share the same decoder/Braid obligations as
    ...
    the generated refusal prefix and the wire/Braid relation are reconstructed
    from the exact `input.hcall` at the provider boundary. -/
structure InitialRatchetTripleBranchContracts
    {R : Type} {rc : rand_core_1.RngCore R} {crc : rand_core_1.CryptoRng R}
    {trace : R → List Model.Lifecycle.Key} {dh : DhView} {K : Model.Braid.Kem}
    {view : Model.Lifecycle.CodewordView} {oracle : Model.Lifecycle.Oracle}
    {real : lifecycle.Session} {model : Model.Lifecycle.Session}
    {inputMessage : Slice Std.U8} {inputRng : R}
    (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
      (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
...
structure InitialRatchetAeadConcreteProviders
    ...
    (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
      (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
      (real := real) (model := model) message rng) where
  aead : ∀ (composite : Model.CompositeHeader.Composite) (ciphertext : Bytes)
      (draw : Model.Lifecycle.Key) (oracleNext : Model.Lifecycle.Oracle)
      (messageKey : Model.Lifecycle.Key)
      (dhOutRecv dhOutSend : Model.Lifecycle.Key)
      (tripleCandidate : Model.Triple.State),
...
noncomputable def initial_ratchet_triple_providers_of_branch_contracts
    ...
    (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
      (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
      (real := real) (model := model) message rng)
    (contracts : InitialRatchetTripleBranchContracts input) :
    InitialRatchetTripleConcreteProviders input := by
  refine { triple := ?_ }
  intro composite ciphertext draw oracleNext modelReason dhOutRecv dhOutSend
    hdecode hfirst hdraw hsecond htriple
  obtain ⟨realReason, ev⟩ := contracts.branch composite ciphertext draw oracleNext
    modelReason dhOutRecv dhOutSend hdecode hfirst hdraw hsecond htriple
  exact ⟨realReason, ev.hreason, ev⟩

noncomputable def initial_ratchet_aead_providers_of_branch_contracts
    {R : Type} {rc : rand_core_1.RngCore R} {crc : rand_core_1.CryptoRng R}
    {trace : R → List Model.Lifecycle.Key} {dh : DhView} {K : Model.Braid.Kem}
    {view : Model.Lifecycle.CodewordView} {oracle : Model.Lifecycle.Oracle}
    {real : lifecycle.Session} {model : Model.Lifecycle.Session}
    ...
    (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
      (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
      (real := real) (model := model) message rng)
    (contracts : InitialRatchetAeadBranchContracts input) :
    InitialRatchetAeadConcreteProviders input := by
  refine { aead := ?_ }
  intro composite ciphertext draw oracleNext messageKey dhOutRecv dhOutSend tripleCandidate
    hdecode hfirst hdraw hsecond htriple haead
  exact contracts.branch composite ciphertext draw oracleNext messageKey dhOutRecv dhOutSend
    tripleCandidate hdecode hfirst hdraw hsecond htriple haead

/-! Turn the concrete Triple evidence into the exact callback consumed by the
    result-shaped branch dispatcher.  The equality in the evidence package is
    used before constructing the indexed route, so the generated refusal tag
    and successor cannot be replaced by a caller-chosen Triple result. -/
noncomputable def initial_ratchet_triple_route_provider_of_concrete_evidence
    {R : Type} {rc : rand_core_1.RngCore R} {crc : rand_core_1.CryptoRng R}
    {trace : R → List Model.Lifecycle.Key} {dh : DhView} {K : Model.Braid.Kem}
    {view : Model.Lifecycle.CodewordView} {oracle : Model.Lifecycle.Oracle}
    {real : lifecycle.Session} {model : Model.Lifecycle.Session}
    ...
    (htrace : trace rng = oracle.draws)
    (hready : Model.Lifecycle.agreementFailed model = false)
    (evidence : InitialRatchetTripleConcreteProviders input) :
  ...
  intro composite ciphertext draw oracleNext modelReason dhOutRecv dhOutSend
    hdecode hfirst hdraw hsecond htriple
  obtain ⟨realReason, hreason, ev⟩ := evidence.triple composite ciphertext draw oracleNext
    modelReason dhOutRecv dhOutSend hdecode hfirst hdraw hsecond htriple
  have hcall' : lifecycle.Session.decrypt_ratchet rc crc real
      input.decoded.message.deref rng =
      ok (.Err (.Triple realReason), input.next, input.rngNext) := by
    simpa [hreason] using input.hcall
  have hroute := initial_ratchet_triple_refusal_route_of_result
    rc crc trace dh kem K view oracle oracleNext oracleOf real model
...
noncomputable def initial_ratchet_aead_route_provider_of_concrete_evidence
    {R : Type} {rc : rand_core_1.RngCore R} {crc : rand_core_1.CryptoRng R}
    {trace : R → List Model.Lifecycle.Key} {dh : DhView} {K : Model.Braid.Kem}
    {view : Model.Lifecycle.CodewordView} {oracle : Model.Lifecycle.Oracle}
    {real : lifecycle.Session} {model : Model.Lifecycle.Session}
    ...
    (htrace : trace rng = oracle.draws)
    (hready : Model.Lifecycle.agreementFailed model = false)
    (evidence : InitialRatchetAeadConcreteProviders input) :
    ∀ (composite : Model.CompositeHeader.Composite) (ciphertext : Bytes)
      (draw : Model.Lifecycle.Key) (oracleNext : Model.Lifecycle.Oracle)
      (messageKey : Model.Lifecycle.Key)
      (dhOutRecv dhOutSend : Model.Lifecycle.Key)
  ...
  intro composite ciphertext draw oracleNext messageKey dhOutRecv dhOutSend tripleCandidate
    hdecode hfirst hdraw hsecond htriple haead
  let ev := evidence.aead composite ciphertext draw oracleNext messageKey dhOutRecv
    dhOutSend tripleCandidate hdecode hfirst hdraw hsecond htriple haead
  have hcall' : lifecycle.Session.decrypt_ratchet rc crc real
      input.decoded.message.deref rng =
      ok (.Err .Aead, input.next, input.rngNext) := by
    simpa [ev.hreason] using input.hcall
  have hroute := initial_ratchet_aead_refusal_route_of_result
    rc crc trace dh kem K view oracle oracleNext oracleOf real model
...
structure InitialRatchetConcreteBranchEvidence
  ...
  aeadContracts : ∀ {innerMessage : Slice Std.U8} {innerRng : R}
      (input : InitialRatchetRefusalBranchInput (rc := rc) (crc := crc)
        (trace := trace) (dh := dh) (K := K) (view := view) (oracle := oracle)
        (real := real) (model := model) innerMessage innerRng),
...
/-! Variant of the public splitter that starts from one exact concrete
    ...
    callback.  Branch providers still remain explicit because they discharge
    the primitive-specific DH, Triple and AEAD contracts. -/
theorem initial_ratchet_refines_of_t1_result_split_with_refusal_step_evidence
    {R : Type} {rc : rand_core_1.RngCore R} {crc : rand_core_1.CryptoRng R}
    {trace : R → List Model.Lifecycle.Key} {dh : DhView} {K : Model.Braid.Kem}
    {view : Model.Lifecycle.CodewordView} {oracle : Model.Lifecycle.Oracle}
    {real : lifecycle.Session} {model : Model.Lifecycle.Session}
        ...
        ∃ oracleNext, ∃ facts : InitialRatchetModelSuccessFacts view oracle oracleNext
          ...
          Nonempty (∀ successPrefix : InitialRatchetSuccessPrefix rc crc real
            ...
            InitialRatchetSuccessSplice (dh := dh) (K := K) (trace := trace)
              successPrefix facts)) :
    InitialRatchetRefines rc crc trace dh K view oracle real model message rng := by
  apply initial_ratchet_refines_of_t1_result_split_with_route_evidence boundary headroom
    hz32 hzKeys
  · intro decoded established hdecode hestablished he hi reason next rngNext hcall
    by_cases hfailed : Model.Lifecycle.agreementFailed model = true
    · obtain ⟨hterminal, hstep⟩ := decrypt_ratchet_terminal_guard_step_refines rc crc
      ...
      · let hnotbad : ∀ realReason,
        ...
        exact Classical.choice (initial_ratchet_nonterminal_route_of_model_result hready
          ...
          (fun decoded established hdecode hestablished he hi reason next rngNext hnotbad hcall =>
            let evidence := hrefusal decoded established hdecode hestablished he hi
              reason next rngNext hnotbad hcall
            initial_ratchet_model_refusal_case_of_step
              (real := real) (model := model) (message := decoded.message.deref)
              (reason := reason) (next := next) (rng := rng) (rngNext := rngNext)
              hready hnotbad evidence.hstep)
          hproviders decoded established hdecode hestablished he hi reason next rngNext
          hnotbad hcall)
  · exact hsuccess
...
theorem public_session_decrypt_end_to_end
  ...
  | initialTerminal codec ctx hfailed =>
      exact decrypt_initial_terminal_refines codec ctx hfailed
  | initialMalformed codec ctx hready hbad =>
      exact decrypt_initial_refines_from_ratchet codec ctx
