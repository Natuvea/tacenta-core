...
| `LAST_RESORT_HANDSHAKE_LABEL` (session) | `tacenta last-resort handshake v1`: 32 ASCII bytes, no terminator | ours | The HMAC-SHA256 key of the last-resort replay fingerprint (session-establishment.md, The fingerprint; `sessions/lifecycle.rs`; registered in `tacenta-core/LABELS.md`). Not secret: it separates the fingerprint from other digests over overlapping bytes. It predates, and does not follow, the registry's shape for new labels. Changing it changes every fingerprint, so a store persisted under the old label would stop refusing the replays it records. The published specifications have no such record. Free choice. |
| `GROUP_ROSTER_COMMITMENT_LABEL` | `Tacenta:group:roster-commitment:v1\xff` | ours | Prefix of the SHA-256 input for a product-canonical bounded group roster preimage (group-messaging.md, Bounded fan-out commitment helper; `groups.rs`; registered in `tacenta-core/LABELS.md`). The source material fixes no group commitment construction. |
| `GROUP_PAYLOAD_COMMITMENT_LABEL` | `Tacenta:group:payload-commitment:v1\xff` | ours | Prefix of the SHA-256 input for a product-canonical bounded group application context (group-messaging.md, Bounded fan-out commitment helper; `groups.rs`; registered in `tacenta-core/LABELS.md`). The source material fixes no group commitment construction. |
