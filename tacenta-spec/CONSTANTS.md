...
| **Our own** Braid MAC length | 32 bytes: the full HMAC-SHA256 output, not truncated | ours | `MAC_SIZE` in the ML-KEM Braid specification §2.2, which recommends HMAC-SHA256 and leaves the size to the implementation (`braid/src/lib.rs`, `MAC_LEN`; `Model/Braid.lean`, `macSize`). Appended to the 64-byte header and to the 160-byte `ct2`, giving the 96- and 192-byte values the erasure code carries (mlkem-braid.md, Parameters and derivations). |
| **Our own** Braid epoch encoding | `ToBytes(epoch)`: 8 bytes, big-endian, of an unsigned 64-bit epoch | ours | §2.2 recommends a 64-bit unsigned epoch and a big-endian `ToBytes`, and fixes neither. The same eight bytes are the composite header's `ag_epoch` and the epoch in every Braid derivation and MAC input (`braid/src/lib.rs`, `info`; `Model/Braid.lean`, `epochBytes`; mlkem-braid.md, Parameters and derivations). |
...
| Constant | Value | Tier | Provenance |
|---|---|---|---|
| `INVENTORY_DOMAIN` | `Tacenta Inventory Statement v1` | ours | Prefix of every unsigned hosted device-inventory statement; the version-one encoding and refusal rules are fixed in `identities-and-devices.md`, Hosted device-inventory statements, and implemented by `groups/inventory.rs`. |
...
| Braid `info` and MAC-input layout | `PROTOCOL_INFO ‖ suffix ‖ ToBytes(epoch)`, nothing between the parts; a MAC input continues with the header, or `ct1 ‖ ct2` | fact | The ML-KEM Braid specification §2.2 states this layout for `KDF_AUTH` and `KDF_OK`, and §2.4 for `MacHdr` and `MacCt`. §2.4 writes the epoch bare in the MAC inputs; this implementation encodes it there with `ToBytes`, as in the derivations (`braid/src/lib.rs`, `info`; `Model/Braid.lean`, `Auth.macHdr`, `Auth.macCt`; mlkem-braid.md, Parameters and derivations, which gives every part's bytes). |
| Braid derivation salts and lengths | `KDF_OK`: salt 32 zero bytes, 32 bytes out. `KDF_AUTH`: salt the current `root_key`, 64 bytes out, the first 32 the new `root_key` and the last 32 the new `mac_key` | fact | §2.2 states both salts and both lengths, and §2.4 assigns `KDF_AUTH`'s output to `root_key` then `mac_key` (`braid/src/lib.rs`, `kdf_ok`, `Auth::update`; `Model/Braid.lean`, `kdfOk`, `Auth.update`). Pinned by `vectors/post-quantum/braid.json` and `auth.json`. |
| Braid authenticator initial state | `root_key` 32 zero bytes, updated at epoch 1 | fact | §2.4 `Authenticator.Init` and §2.6 `InitAlice`, `InitBob` (`braid/src/lib.rs`, `Auth::init`, `Braid::initiator`, `Braid::responder`; `Model/Braid.lean`, `Auth.init`, `initAlice`, `initBob`). The `from-zero` vector in `auth.json` is this initialisation. |
| Braid preshared secret | the PQXDH `SK`, whole | ours | §2.6 takes a preshared secret that "may come from a handshake protocol such as PQXDH". Here it is `SK` itself, not either half of the Triple Ratchet's split (`sessions/lifecycle.rs`, `Braid::initiator(&sk)`, `Braid::responder(&sk)`; mlkem-braid.md, Parameters and derivations; session-establishment.md). |
...
| `LAST_RESORT_HANDSHAKE_LABEL` (session) | `tacenta last-resort handshake v2`: 32 ASCII bytes, no terminator | ours | The HMAC-SHA256 key of the last-resort replay identity, which is computed over the agreed `SK` rather than public initial-message bytes (session-establishment.md, The replay identity; `sessions/lifecycle.rs`; registered in `tacenta-core/LABELS.md`). Not secret: it separates the identity from other digests over overlapping bytes. Changing it changes every identity, so v1-v4 stores fail closed for live last-resort keys until rotation. The published specifications have no such record. Free choice. |
| `INVENTORY_SIGNING_LABEL` | `Tacenta:inventory-statement:v1\xff` | ours | Prefix of the XEdDSA input for a hosted device-inventory statement (`identities-and-devices.md`, Hosted device-inventory statements; `groups/inventory.rs`; registered in `tacenta-core/LABELS.md`). |
| `BINDING_COMMITMENT_LABEL` | `Tacenta:inventory-binding-commitment:v1\xff` | ours | Prefix of the SHA-256 input for the exact canonical binding a replacement retires (`identities-and-devices.md`, Hosted device-inventory statements; `groups/inventory.rs`; registered in `tacenta-core/LABELS.md`). |
...
| `PREKEY_STORE_VERSION` | `0x05` written; `0x04`, `0x03`, `0x02` and `0x01` accepted on read | ours | The prekey store's own format, at the session layer (`sessions/lifecycle.rs`). v5 stores `SK`-bound last-resort replay identities and fail-closed key identifiers for nonempty v1-v4 records; v4 tagged public-byte fingerprints with the KEM key they were made against; v3 added the retired prekeys a rotation keeps; v2 added the replay record; v1 is the original. A nonempty v1-v4 record is not comparable to v5 identities and therefore blocks its live last-resort keys until rotation; a v1 store has no record and reads back with none remembered (session-persistence.md, Prekey store). |
...
| `GROUP_EPOCH_V1` | `1` | ours | The only capability bit defined by the version-one hosted inventory profile (`identities-and-devices.md`, Hosted device-inventory statements; `groups/inventory.rs`). Other bits are refused until a later profile defines them. |
| `MAX_ACCOUNT_BYTES` | `256` | ours | Maximum UTF-8 byte length of an inventory account handle in the version-one hosted profile (`identities-and-devices.md`; `groups/inventory.rs`). |
| `MAX_ACTIVE_BINDINGS` | `8` | ours | Maximum number of active device bindings in one version-one inventory statement (`identities-and-devices.md`; `groups/inventory.rs`). |
| `MAX_RECENT_REVOCATIONS` | `8` | ours | Maximum number of recent revocation records in one version-one inventory statement (`identities-and-devices.md`; `groups/inventory.rs`). |
