...
Braid-role disagreements, the unanswered-initiator/responder exclusion, and one
Rust-backed `ratchet_private` mismatch, nested half-reader refusals,
boundary-neighbour mutations, or truncations, additions and a version
relabelling.
...
- An invalid vector's `refusal` is `wrong-version`, `short-or-malformed`,
  ...
  `triple-state-reader-refuses` and `braid-reader-refuses` change the nested
  half's own version byte and are refused as `short-or-malformed`, not as the
  nested `wrong-version`. `unanswered-initiator-is-also-responder` is generated
  by Lean from a responder fixture carrying the pending initial from the
