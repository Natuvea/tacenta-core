...
| Prekey-store lifecycle structural effects | key-deletion.md; session-persistence.md, Prekey store; ASM-19 P6 operation model | concrete Rust operations are checked by `check prekey`: publication leaves the store unchanged; replenishment appends curve and KEM one-time ids from `next_id`; signed-prekey rotation retains the previous signed prekey; KEM rotation retains the previous KEM key and prunes the older key's replay records; identity-mismatch lifecycle calls are checked as no-ops |
