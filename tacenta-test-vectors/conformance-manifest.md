...
- **Specification:** tacenta-spec/protocol/session-establishment.md, written from
  ...
  **revision 3, 2023-05-24, last updated 2024-01-23**, with X3DH revision 1 for
  ...
  SHA-256.
- **Oracles:** `pqxdh-sk.json` is generated by tacenta-model
  (`Model.SessionEstablishment`). `session-e2e.json` is a project-generated
  fixed known answer: the Rust runner drives the public session lifecycle and
  also checks a composition path that reconstructs its initial message from
  the public leaf components; both paths use the same live crates, so this is
  composition evidence rather than an independent implementation. It is a
  regression and composition oracle, not an external implementation.
...
| Associated data construction | Sending the initial message | core unit tests; `session-e2e.json` pins the identity AD and the whole `CONCAT(ad, header)` value |
| The four Diffie-Hellman agreements, both sides | Sending / receiving the initial message | core tests: initiator and responder derive the same secret, with and without a one-time curve prekey; `session-e2e.json` pins DH1 through DH4 separately |
...
| The full handshake, both sides | The PQXDH protocol | core test with a real KEM prekey; `session-e2e.json` pins the encoded bundle, ML-KEM ciphertext and secret, PQXDH secret, initial message, consumed prekey store and both resulting session states |
| Composition into the ratchet | Replay (why a ratchet must follow) | `tests/handshake_to_ratchet.rs`: the secret seeds the ratchet and a message round-trips through the AEAD; `session-e2e.json` pins the split halves, first classical and sparse keys, combined key, header, AEAD output and responder plaintext |
| Known-answer check is live | The complete path above | `session_end_to_end_control_rejects_wrong_associated_data` changes the committed associated-data answer and requires the runner to fail on that field |
...
| Ratchet state counter ceilings, by operations: the clock stays at its stop, and a step past `ns` or `nr` at `u32::MAX` is refused as `counter-exhaustion` | ratchet.md, Sending and receiving and Skipped keys; session-persistence.md, Principles | same file: from `events` at `u32::MAX - 1`, a receive from the store and one on the chain, `events` staying at `u32::MAX - 1`; from `ns` at `u32::MAX`, a send; from `nr` at `u32::MAX`, the message numbered `u32::MAX` |
...
| Sparse ratchet state counter ceilings, by operations: the advance onto epoch `u64::MAX`, and a step past a chain's counter at `u64::MAX`, each refused as `counter-exhaustion` | sparse-pq-ratchet.md, Sending and Receiving; session-persistence.md, Principles | same file: from epoch `u64::MAX - 1`, a receive carrying epoch `u64::MAX`'s secret; from a sending chain at `u64::MAX`, a send; from a receiving chain at `u64::MAX`, message `u64::MAX` |
...
`TooShort` and `Malformed` are `short-or-malformed`. An invalid operations
vector's steps are replayed up to the last, which the crate must refuse with
`ChainExhausted`, the vector's `counter-exhaustion`.
...
One persisted-format rule is outside the Lean model generator:

- the session's first semantic rule, that `ratchet_private`'s public key equals
  the classical ratchet's `dhs_pub`, needs X25519. The vector
  `session-state/ratchet-private-does-not-match-dhs-pub` is appended after
  generation by `augment-session-state.py` and checked by readers that compute
  the curve.
...
- **The session semantic rules still not reached by vectors.** Vectors reach eight:
  the epoch relation (both the tag 6 `e - 1` side, the tag 7 `e` side and the
  failed tag 11 exemption), the ratchet-private/public-key relation, the
  associated data's orientation, the role agreement (both its sparse and Braid
...
implementation (session-persistence.md, Rejection), `Model.PersistedState`
reads the version byte first and both crates check the length first, and no
vector has one. And a store of exactly 2,000 keys, the accepted side of
`MAX_SKIPPED_STORE`: its vector would be about 290 kilobytes, and the refused
side is pinned. In place of a
...
| Prekey-store lifecycle and replay structural effects | key-deletion.md; session-persistence.md, Prekey store; ASM-19 P6 operation model | concrete Rust operations are checked by `check prekey`: publication leaves the store unchanged; replenishment appends curve and KEM one-time ids from `next_id`; signed-prekey rotation retains the previous signed prekey; KEM rotation retains the previous KEM key and prunes the older key's replay records; authenticated responder establishment deletes the named one-time curve and KEM ids or appends one last-resort replay fingerprint; identity-mismatch lifecycle calls and replay refusals are checked as no-ops |
| Initial session-establishment structural effects | session-establishment.md; session-persistence.md, Session; ASM-19 P6 operation model | concrete Rust establishment is checked by `check session`: an identity-pinned initiator persists as pending, and a responder after an authenticated initial message persists as established; both sessions retain the expected identity binding and satisfy the composed session invariant. An unexpected bundle identity refuses before session-creation randomness, and a forged initial AEAD tag returns no session and leaves the prekey store unchanged. Signature, KEM and non-contributory-DH refusals remain outside this operation check. |
| The session's shared stored-format domain | session-persistence.md, Session; Rejection | committed P4 fixtures for responder/initiator roles, optional fields, tag 6/7 epoch boundaries, failed-Braid exemption, role agreement, inner-reader refusals, wrong-version, short-or-malformed and inconsistent refusals are given to `read session`; accepted imports compare canonical re-encoding, and the run asserts accepted, wrong-version, malformed, inconsistent and excluded cases were reached |
...
- **The cryptographic relations outside the model domain.** The prekey store
  and session are driven through their shared structural domains. The prekey
  store's stored-signature refusal, the FIPS 203 checks inside KEM key pairs,
  and the session's ratchet-private/public relation remain outside that
  comparison because `Model.PersistedState` deliberately has neither
  signatures, KEM arithmetic nor X25519 public-key computation; the persisted
  vectors and crate tests pin those rules separately.
...
establishment: the current vector reaches one real X25519/ML-KEM handshake and
the first authenticated message. It does not yet cover the last-resort KEM
path, the no-one-time-curve-prekey path at byte level, a second application
message, or the refusal cases; core tests cover parts of those paths. See the
PQXDH section above for the exact boundary. Malformed-input handling is covered by vectors for
