...
| PROOF-ERASURE-REFINEMENT | Deferred | BLOCKING | P7 | Proof scope | Field arithmetic and panic-freedom are covered; full codec refinement remains outside current L3 evidence while the erasure target remains L4. `tacenta-proofs/ERASURE-DECODER-REFINEMENT.md` records the opaque-boundary gap. The duplicate/completed model effects and the completed translated leaf-decoder effect are proved, but Braid's erasure dependency remains separately translated and opaque. | Link the leaf and Braid symbols, bridge duplicate and completed observations through the real translated decoder API, prove the corresponding concrete `DecoderRefines` preservation statement, then either complete L4 evidence or record a target/scope decision. |
