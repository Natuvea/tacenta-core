...
/-- Decode exactly one composite header. `Model.CompositeHeader.decode` is a
    prefix parser because ratchet messages put ciphertext after the header;
    `composite-header-decode` vectors exercise the standalone decoder contract. -/
@[never_extract]
def decodeCompositeExact (encoding : List UInt8) : Option Model.CompositeHeader.Composite := do
  let (h, rest) ← Model.CompositeHeader.decode encoding
  if rest.isEmpty then some h else none

/-- A composite-header decoder vector, written only when the model's standalone
    decoder accepts the encoding exactly when `accept` says it does. -/
@[never_extract]
def compositeDecodeBytesVector (id comment : String) (encoding : List UInt8)
    (accept : Bool) : Except String String :=
  match decodeCompositeExact encoding, accept with
  | some d, true =>
    .ok (decodeVector id comment encoding (some (Model.CompositeHeader.encode d)))
...
def compositeDecodeVector (id comment : String) (h : Model.CompositeHeader.Composite)
    (accept : Bool) : Except String String :=
  compositeDecodeBytesVector id comment (Model.CompositeHeader.encode h) accept
...
def compositeDecodeFile (_ : Unit) : Except String String := do
  let vectors ← [
    ...
    compositeDecodeVector "dh-equal-to-p"
      ...
      { compA with dh := pItself } false,
    compositeDecodeBytesVector "trailing-byte"
      "one byte after the fixed-width composite header is refused by the standalone decoder"
      (Model.CompositeHeader.encode compA ++ [0x00]) false].mapM id
  pure ("{\n" ++
    "  \"schema_version\": 1,\n" ++
    "  \"algorithm\": \"composite-header-decode\",\n" ++
    "  \"source\": \"generated by tacenta-model Vectors.lean (lake exe genvectors composite-decode), from the standalone exact composite-header decoder layered over Model.CompositeHeader.decode; an accepted vector's output is the re-encoding of the header it decodes to, and the generator writes no vector whose result the model does not give\",\n" ++
    "  \"vectors\": [\n" ++
    String.intercalate ",\n" vectors ++
    "\n  ]\n}")
...
/-- As `storedStateVector`, and additionally requires that an accepted state is
    written back as the bytes it was read from.
...
def storedStateVectorWritten {σ : Type}
  ...
  match reader bs, expect with
  | .ok st, none =>
    if writer st == bs then .ok (fieldsVector id comment "bytes" bs (some (fieldsOf st)))
    else .error ("genvectors: " ++ id ++
      " is accepted but is not written back as the bytes it was read from")
  | _, _ => storedStateVector reader fieldsOf id comment bs expect
...
`tacenta-core` refuses one whose signatures do not. The four accepted vectors
...
def prekeyFixtureV5 (hex : String) : String :=
  match Model.PersistedState.PrekeyStoreState.ofBytes (ofHex hex) with
  | .ok st => toHex (Model.PersistedState.PrekeyStoreState.toBytes st)
  | .error _ => hex

-- The no-one-time fixture is kept in the source as the reviewed v4 export;
-- current vectors use its canonical v5 spelling, while the legacy vectors
-- need the original v4 layout as their starting point.
def prekeyFixtureV4 (hex : String) : String :=
  let bs := ofHex hex
  toHex (replaceAt (bs.take 4957 ++ bs.drop 4961) 0 0x04)
...
def prekeyFixture_one_one_time : String :=
  prekeyFixtureV5 ""

def prekeyFixture_retired_signed : String :=
  prekeyFixtureV5 ""

def prekeyFixture_retired_kem : String :=
  prekeyFixtureV5 ""
...
def prekeyStored (id comment : String) (bs : List UInt8)
    (expect : Option Model.PersistedState.Refusal) : Except String String :=
  storedStateVectorWritten Model.PersistedState.PrekeyStoreState.ofBytes
    Model.PersistedState.PrekeyStoreState.toBytes prekeyStoreFields id comment bs expect

/-- An incoherent prekey-store fixture is structurally valid to the model and
    writes back unchanged, but carries a signature mutation only a cryptographic
    runner can reject. This is the prekey store's one semantic rule outside the
    model boundary. -/
@[never_extract]
def prekeyStoredIncoherent (id comment : String) (bs : List UInt8) : Except String String :=
  match Model.PersistedState.PrekeyStoreState.ofBytes bs with
  | .ok st =>
    if Model.PersistedState.PrekeyStoreState.toBytes st == bs then
      .ok (vectorHead id comment ++ "\"result\": \"invalid\", \"refusal\": \"incoherent\", \"inputs\": " ++ jsonObject [("bytes", toHex bs)] ++ " }")
    else .error ("genvectors: " ++ id ++
      " is structurally accepted but is not written back as the bytes it was read from")
  | .error r => .error ("genvectors: " ++ id ++
      " should reach the signature rule, but the model refuses it structurally as " ++
      refusalName r)

/-- Accepted legacy stores read to the same fields, but are written back in the
    current v4 format. -/
@[never_extract]
def prekeyStoredLegacy (id comment : String) (bs : List UInt8) : Except String String :=
  storedStateVector Model.PersistedState.PrekeyStoreState.ofBytes prekeyStoreFields id comment bs none

/-- Re-spell a current no-record, no-retired prekey-store fixture as one of the
    older accepted layouts. v3 has the same tail in that case; v2 lacks the two
    retired-presence bytes; v1 lacks those and the replay-record count. -/
def prekeyLegacyBytes (version : UInt8) (bs : List UInt8) : List UInt8 :=
  let body := bs.drop 1
  if version = Model.PersistedState.PrekeyStoreState.versionV3 then
    version :: body
  else if version = Model.PersistedState.PrekeyStoreState.versionV2 then
    version :: body.take (body.length - 2)
  else
    version :: body.take (body.length - 6)
...
def prekeyStoreStateFile (_ : Unit) : Except String String := do
  ...
  let legacyBaseBytes := ofHex (prekeyFixtureV4 prekeyFixture_no_one_time)
  let accepted ← [
    ...
    prekeyStored "retired-signed-prekey"
      "a store after a signed-prekey rotation, so the retired signed-prekey fields are present"
      (ofHex prekeyFixture_retired_signed) none,
    prekeyStored "retired-kem-prekey"
      "a store after a KEM prekey rotation, so the retired KEM sub-format is present"
      (ofHex prekeyFixture_retired_kem) none
  ].mapM id
  let refusals := [
    -- The version byte.
    prekeyStored "version-unknown" "a first byte naming no version this reader reads"
      (0x06 :: baseBytes.drop 1) (some .wrongVersion),
    ...
    prekeyStored "record-at-budget"
      ...
      (mutate (fun s => { s with
        seen := (List.range Model.PersistedState.PrekeyStoreState.maxLastResortSeen).map
          (fun i => (s.kemId, Model.Erasure.be 32 i)) }))
      none,
    prekeyStoredIncoherent "signed-prekey-signature-does-not-verify"
      "signed_prekey_sig has one byte flipped in a fixture whose parser, KEM-pair checks and structural semantic rules still pass"
      (replaceAt baseBytes 69 ((baseBytes.getD 69 0) ^^^ 0x01))
  ].mapM id
  let refusals ← refusals
  pure ("{\n" ++
    "  \"schema_version\": 1,\n" ++
    "  \"algorithm\": \"prekey-store-state\",\n" ++
    "  \"source\": \"generated by tacenta-model Vectors.lean (lake exe genvectors prekey-store-state), from Model.PersistedState.PrekeyStoreState; accepted cryptographic fixtures carry bytes tacenta-core produced under a fixed byte source, because the model has no signatures and cannot make a store whose signatures verify; legacy v1/v2/v3 accepted vectors are the no-record, no-retired fixture re-spelled in the older layouts and read as the same fields before upgrading to v5 on write-back; every refusal that changes a field changes one, so the rule under test is the only thing wrong; the rest are truncations, additions and version relabellings; the stored-signature refusal is emitted only after the model accepts and re-encodes the mutated bytes, because signature verification is outside the model boundary\",\n" ++
    "  \"vectors\": [\n" ++
    String.intercalate ",\n" (accepted ++ refusals) ++
    "\n  ]\n}")
...
def sessionStored (id comment : String) (bs : List UInt8)
    (expect : Option Model.PersistedState.Refusal) : Except String String :=
  storedStateVectorWritten Model.PersistedState.SessionState.ofBytes
    Model.PersistedState.SessionState.toBytes sessionFields id comment bs expect
...
def sessionWithFailedBraid (s : Model.PersistedState.SessionState.Session)
    (sparseEpoch : Nat) : Model.PersistedState.SessionState.Session :=
  { s with
    triple := { s.triple with
      postQuantum := sessionSparseAtEpoch s.triple.postQuantum sparseEpoch },
...
def sessionStateFile (_ : Unit) : Except String String := do
  ...
  let accepted ← [
    ...
    sessionStored "failed-braid-exempts-sparse-epoch"
      "Failed carries no epoch, so the session accepts it even though the sparse epoch is 1"
      (mutate (fun s => sessionWithFailedBraid s 1)) none
  ].mapM id
  let refusals ← [
    ...
    sessionStored "trailing-byte" "a byte after the last field"
      (baseBytes ++ [0x00]) (some .shortOrMalformed),
    sessionStored "triple-state-reader-refuses"
      "the nested triple_state has its own version byte changed, so the session refuses it as short-or-malformed rather than passing through the inner wrong-version"
      (baseBytes.set 5 0x02) (some .shortOrMalformed),
    sessionStored "braid-reader-refuses"
      "the nested Braid has its own version byte changed, so the session refuses it as short-or-malformed rather than passing through the inner wrong-version"
      (baseBytes.set braidStart 0x02) (some .shortOrMalformed),
    ...
    sessionStored "associated-data-wrong-orientation"
      ...
      (some .inconsistent),
    sessionStored "halves-disagree-on-the-sparse-role"
      "the sparse ratchet's direction flipped, so the sparse half no longer agrees on which side this is"
      (mutate (fun s => { s with triple := { s.triple with
        postQuantum := { s.triple.postQuantum with
          direction := Model.SparseRatchet.Direction.a2b } } }))
      (some .inconsistent),
    sessionStored "halves-disagree-on-the-braid-role"
      "tag 6 at Braid epoch 2 still matches sparse epoch 1, but its tag/parity role is the initiator's, not the responder's"
      (mutate (fun s => sessionWithBraidBoundary s 6 2 1 tag6Fields))
      (some .inconsistent),
    sessionStored "unanswered-initiator-is-also-responder"
      "the responder fixture also carries a valid pending_initial, so the stored session is both an unanswered initiator and a responder"
      (mutate (fun s => { s with pendingInitial := pendingBase.pendingInitial }))
      (some .inconsistent),
  ...
  pure ("{\n" ++
    ...
    "  \"source\": \"generated by tacenta-model Vectors.lean (lake exe genvectors session-state), from Model.PersistedState.SessionState; the first three accepted vectors carry sessions tacenta-core exported under the counter-based FixedRng in lifecycle.rs, because the model does not compute the curve and so cannot build a session whose ratchet_private matches the classical ratchet's dhs_pub; the tag 6/7 boundary neighbours, failed-Braid exemption vector, Braid-role refusal and unanswered-role refusal reuse the responder fixture's cryptographic material while changing only model-structural Braid, sparse epoch or optional-role fields; the nested-half refusal vectors change the inner version byte of the triple_state or Braid field and are refused before session semantic rules run; every refusal that changes a field changes one, so the rule under test is the only thing wrong; the rest are truncations, additions and a version relabelling; the semantic rules carry the refusal inconsistent, which the page distinguishes from malformed; the generator writes no vector whose result the model does not give\",\n" ++
    "  \"vectors\": [\n" ++
    String.intercalate ",\n" (accepted ++ refusals) ++
    "\n  ]\n}")
