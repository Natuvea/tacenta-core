...
| Prekey store (v1 through v5 read, v5 written; the one-time, one-time-KEM and retired sub-formats; the replay record and fail-closed legacy markers) | `PrekeyStoreState.toBytes`, `PrekeyStoreState.ofBytes`, `readSeen`, `readLegacyBlocked`, `readPrev`, `readKemOneTimes` | `Model/PersistedState.lean` |
...
| Executable lifecycle properties; success-only pending clearing, exact repeat recognition, terminal-failure stickiness, session/store refusal no-ops and the responder's exact success-side store effect | `pending_cleared_iff_ok`, `repeated_initial_accepted_iff`, `agreement_failed_encrypt_sticky`, `agreement_failed_decrypt_sticky`, `refusal_is_no_op`, `responder_refusal_is_no_op`, `responder_success_store_effect` | `Properties/Lifecycle.lean` |
| Two-party schedule mechanics; send, delivery, loss, replay insertion, reordering, byte forgery and forced terminal agreement state | `LifecycleTrace.State`, `LifecycleTrace.Action`, `LifecycleTrace.step`, `LifecycleTrace.run` | `Model/LifecycleTrace.lean` |
| Schedule frame conditions; refused delivery preserves the recipient Session, network-only actions preserve both Sessions, and terminal agreement failure remains terminal through every later scheduled action | `refused_receive_keeps_alice`, `refused_receive_keeps_bob`, `network_action_keeps_sessions`, `failed_step_stays_failed_alice`, `failed_step_stays_failed_bob`, `failed_run_stays_failed_alice`, `failed_run_stays_failed_bob` | `Properties/LifecycleTrace.lean` |
| Executable Session integration checks across initiator and responder establishment, Braid, Triple, initial and ratchet wire codecs, a key-and-AD-sensitive AEAD boundary, responder replay recording and first plaintext delivery | `toyViewFor`, `toyOracle`, `toyBundle`, `toyResponderStore`, `toyAlice`, `toyBob` and the closing `native_decide` examples | `Model/Lifecycle.lean` |
