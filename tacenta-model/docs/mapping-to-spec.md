...
| Executable lifecycle properties; success-only pending clearing, exact repeat recognition, terminal-failure stickiness and session/store refusal no-ops | `pending_cleared_iff_ok`, `repeated_initial_accepted_iff`, `agreement_failed_encrypt_sticky`, `agreement_failed_decrypt_sticky`, `refusal_is_no_op`, `responder_refusal_is_no_op` | `Properties/Lifecycle.lean` |
