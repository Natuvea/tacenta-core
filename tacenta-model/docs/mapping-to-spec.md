...
| Schedule frame conditions; refused delivery preserves the recipient Session and network-only actions preserve both Sessions | `refused_receive_keeps_alice`, `refused_receive_keeps_bob`, `network_action_keeps_sessions` | `Properties/LifecycleTrace.lean` |
