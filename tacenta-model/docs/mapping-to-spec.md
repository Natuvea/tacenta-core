...
| Executable lifecycle properties; identity/role frame conditions, success-only pending clearing, exact repeat recognition, terminal-failure stickiness, session/store refusal no-ops, initiator identity/pending shape and the responder's exact success-side store effect | `identityFrame`, `encrypt_frame`, `decryptRatchet_frame`, `decrypt_identity_frame`, `pending_cleared_iff_ok`, `repeated_initial_accepted_iff`, `agreement_failed_encrypt_sticky`, `agreement_failed_decrypt_sticky`, `refusal_is_no_op`, `initiator_success_shape`, `responder_refusal_is_no_op`, `responder_success_store_effect` | `Properties/Lifecycle.lean` |
