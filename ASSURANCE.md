...
| Persisted formats: session | ✓ | ✓ | ✓ seven of the page's eight semantic rules; not the first, that `ratchet_private`'s public half is the classical ratchet's `dhs_pub` (the model does not compute the curve) | ✓ layout, tag 6/7 epoch boundary branches, failed-Braid exemption, Braid-half role agreement, ratchet private/public relation, unanswered-role exclusion, reachable inner invariant refusals and field-level refusal coverage from pass 7 | ✓ pass 7: committed vectors and derived cases; P4 fixture gaps closed, while P5 differential checks remain absent | — | — | L1 | L2 |
...
4. **Differential testing (done for the two ratchets and the Triple Ratchet, partly for the Braid and prekey store):** generated operation sequences run through `tacenta-model` and `tacenta-core`, comparing outcomes, refusals and persisted bytes, in `tooling/ci.sh` and the CI vectors job. The Braid is covered for its decoder and the two transitions that meet its epoch ceiling; the rest of its state machine, and the `key_pair` content clause that the page scopes to implementations knowing the KEM layout ADR-0006, point 5, delegates, are covered by `tacenta-braid`'s own tests instead. The prekey store is now driven over the shared structural stored-format domain; its signature rule and KEM arithmetic remain pinned outside the model by vectors and crate tests. The session stored format is not yet driven by the differential harness.
5. **Model and formats:**
   ...
   - persisted formats phase 3 (the session and the prekey store): modelled and read by the independent reader, with the P4 fixture gaps closed in the gap register; the prekey store's shared structural domain is now compared by the differential harness, while session differential checks remain absent.
