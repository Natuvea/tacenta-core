...
| 4 | Model separate from implementation | Good | The Lean model, its translation, and T3 refinement for the ratchets, Braid, decoders and PQXDH derivation. Vectors pin the rest. All six persisted formats are modelled and pinned by vectors; the prekey store's and the session's carry one cryptographic rule each that the model cannot state, and the rows below say which. P6 now also models selected durable session/prekey operations, but not the full orchestration surface. | Complete the P6 operation matrix and independent-reader coverage before raising the orchestration row. |
...
| 9 | Differential testing | Partial | Model-generated vectors are checked against the Rust, and an independent reader written from the specification alone checks every vector. Generated operation sequences run through both sides for the two ratchets and the Triple Ratchet (`tacenta-model/Difftest.lean` and `tacenta-test-vectors/runners/rust/tests/differential.rs`), comparing the outcome, the persisted bytes and the export-and-import check at every step, the refusal kind on a corrupted import, and the counter ceilings both sides stop at. The Braid's decoder is driven on generated stored states, and its two `Ct2Sampled` transitions, which meet the epoch ceiling, are driven from stored bytes. The prekey store and session are driven over the committed P4 fixtures in their shared structural domains: accepted current and legacy stores, optional branches, record bounds, roles, epoch relations, inner-reader refusals, wrong-version, short-or-malformed and inconsistent refusals, with the cryptographic relations outside the model recorded as exact exclusions. P6 operation checks cover concrete prekey lifecycle/replay, committed initiator/responder states, authenticated send/receive, repeated initial wrappers, third-first delivery with skipped-key recovery, forged/duplicate no-ops, terminal Braid failure and its later refusals. Low-order ratchet-header and responder-establishment controls, plus malformed initial-wire refusal, preserve the relevant persisted state and leave the genuine in-flight message live where applicable. Changed prekey signatures refuse before the first initiator randomness draw, so no session is created. Each comparison has been shown able to fail by mutating the model one thing at a time. This is testing, not proof: it pins the sequences or fixtures a seed reaches and says nothing about the ones it does not. Not driven: P6 independent-reader coverage and the rest of the Braid state machine, which needs the KEM layout ADR-0006 delegates. | P6 independent-reader scenario; P7 Braid state-machine work remains separate. |
...
4. **Differential testing (done for the two ratchets and the Triple Ratchet, partly for the Braid and prekey store):** generated operation sequences run through `tacenta-model` and `tacenta-core`, comparing outcomes, refusals and persisted bytes, in `tooling/ci.sh` and the CI vectors job. The Braid is covered for its decoder and the two transitions that meet its epoch ceiling; the rest of its state machine, and the `key_pair` content clause that the page scopes to implementations knowing the KEM layout ADR-0006, point 5, delegates, are covered by `tacenta-braid`'s own tests instead. The prekey store and session are now driven over their shared structural stored-format domains; stored signatures, KEM arithmetic and the session ratchet-private/public relation remain pinned outside the model by vectors and crate tests.
5. **Model and formats:**
