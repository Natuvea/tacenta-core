...
| 4 | Model separate from implementation | Good | The Lean model, its translation, and named T3 functional-agreement theorems for the ratchets, Braid, decoders and PQXDH derivation. The classical and Triple Ratchet `receive` theorems cover successful Rust results only; the sparse ratchet and named decoders have stronger accepted/refused matching recorded in `CLAIMS.md`. Vectors pin the rest. All six persisted formats are modelled and pinned by vectors; the prekey store's and the session's carry one cryptographic rule each that the model cannot state, and the rows below say which. P6 now covers its selected durable session/prekey operation surface at L2; [its target decision](tacenta-model/P6-L2-TARGET-DECISION.md) keeps the bounded and declared-crypto exclusions explicit. | Reopen P6 on a recorded decision trigger. |
| 5 | Invariants, not examples | Partial | T1 panic-freedom, scoped T3 functional agreement, decoded-state invariants, and forward secrecy and post-compromise security against a symbolic attacker. T3 does not uniformly mean bidirectional or whole-domain refinement: in particular, the classical and Triple Ratchet `receive` theorems say nothing about Rust `Err` branches. Nor has the tree shown every numeric precondition satisfiable: about 140 hypotheses in about 54 shapes remain outside the witness checks, although the decoded-state chains discharge the classical ratchet's store bounds and the sparse ratchet's room bounds. The classical and sparse ratchets' models stop at the counter ceilings the pages state, and persistence vectors pin those ceilings against `tacenta-core`. `Model.Braid` stops at its epoch ceiling too, and its reserved epoch and `Ct2Sampled` boundary transitions are pinned in vectors and the differential harness. `evidence-index.json` now has checked `INV-*` records for the session/prekey invariants, including explicit operation effects and headroom dispositions. [The recorded scope decision](tacenta-proofs/PROOF-BOUNDARY-HEADROOM-TARGET-DECISION.md) keeps the successor-headroom limit on success refinement explicit; it does not claim boundary success refinement. | Add theorem-linked witnesses for every pinned headline theorem's numeric hypothesis set, then remove the affected component markers below. |
| 6 | Verification in CI | Partial | Lean builds, a `collectAxioms` sweep that refuses `sorryAx` even when its warning is disabled, kernel replay, translation checksum checks, vectors current with the model, and an isolated specification-only reader. A planted warning-suppression case must fail the environment audit. Thirteen merged PR heads ([#76](https://github.com/Natuvea/tacenta-core/pull/76)–[#81](https://github.com/Natuvea/tacenta-core/pull/81), [#100](https://github.com/Natuvea/tacenta-core/pull/100), [#101](https://github.com/Natuvea/tacenta-core/pull/101), and [#103](https://github.com/Natuvea/tacenta-core/pull/103)–[#107](https://github.com/Natuvea/tacenta-core/pull/107)) have neither a check run nor an Actions run recorded for their merged head; that history cannot be made green retroactively. Since 2026-09-17, `main` protection requires all ten current contexts on the current PR head, is strict about the base, and applies to administrators. The reader and its pass records are project-controlled evidence, not independent review; the translation checksum does not prove regeneration. | Demonstrate the live current-head rule on the remediation stack and keep every required context meaningful. |
| 7 | Reviewed normative changes | Partial | In the audited post-ADR scope, 32 merged normative PRs ([#77](https://github.com/Natuvea/tacenta-core/pull/77)–[#98](https://github.com/Natuvea/tacenta-core/pull/98), [#100](https://github.com/Natuvea/tacenta-core/pull/100)–[#103](https://github.com/Natuvea/tacenta-core/pull/103), [#105](https://github.com/Natuvea/tacenta-core/pull/105)–[#107](https://github.com/Natuvea/tacenta-core/pull/107), [#109](https://github.com/Natuvea/tacenta-core/pull/109), [#110](https://github.com/Natuvea/tacenta-core/pull/110), and [#129](https://github.com/Natuvea/tacenta-core/pull/129)) have neither a review nor a comment. Every other recorded review was submitted by the change author's account, so it is not independent review. Since 2026-09-17, `main` protection requires one approving account other than the last pusher, dismisses stale approvals, requires conversations resolved, and applies to administrators. `tooling/check-signoff.sh` checks DCO trailers; a DCO sign-off is not a review. Automated or AI-assisted analysis is project-controlled evidence and does not satisfy the required independent approval. | Obtain and retain a claim-by-claim independent approval on the frozen remediation head before merge. |
...
A summary by component. A tick means the component has that kind of evidence,
not that every part of it does. A T3 tick means the named theorem scope in
`CLAIMS.md`; it does not by itself mean bidirectional, whole-domain or
failure-branch refinement. The classical Double Ratchet and composed Triple
Ratchet `receive` entries are success-path-only. The sparse ratchet's
`receive_refines` instead matches model refusal to a Rust error and model
acceptance to a Rust success under its stated hypotheses.

| Component | Tests | Fuzz | Model | Vectors | Reader | T1 | T3 | Level now | Target |
|---|---|---|---|---|---|---|---|---|---|
| Wire decoders | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | L4 | L4 |
| Double Ratchet | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | L4 | L4 |
| Sparse post-quantum ratchet | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | L4* | L4, after theorem-linked numeric witnesses |
| Triple Ratchet | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | L4* | L4, after theorem-linked numeric witnesses |
| ML-KEM Braid | ✓ | ✓ | ✓ | partial (persisted tags 1--4 contain a delegated KEM layout with no universal vector verdict; state-machine controls run separately) | ✓ | ✓ | ✓ | L3/L4* | L4, after theorem-linked numeric witnesses |
| Erasure code | ✓ | ✓ (through the Braid) | ✓ | ✓ | ✓ | ✓ | field only | L3* | L3, by [recorded target decision](tacenta-proofs/ERASURE-CODEC-TARGET-DECISION.md), with numeric hypotheses witnessed |
...
`*` **Numeric-precondition qualification.** The theorem and its stated
hypotheses compile, but the tree has not yet linked a concrete satisfiability
witness to every numeric hypothesis set behind that component's headline T1/T3
claims. `LIMITATIONS.md` records the rough count (about 140 hypotheses on about
95 theorems, in about 54 shapes), the families already discharged from decoded
invariants, and the remaining examples. A star does not mean every state meets
the premise; removing it requires both a witness for the joint hypothesis set
and an explicit account of which reachable states satisfy it.
