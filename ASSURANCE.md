...
| 6 | Verification in CI | Partial | Lean builds, the `sorry` scan, kernel replay, translation checksum checks, vectors current with the model, and an isolated specification-only reader. The reader and its pass records are project-controlled evidence, not independent review; the translation checksum does not prove regeneration. | Close the open hostile-lens gate findings and obtain an independent claim review. |
